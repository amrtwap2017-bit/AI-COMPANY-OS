...
from datetime import datetime
"""
WebhookConfig repository — Triangle Black
"""
import uuid
from datetime import datetime
...
class WebhookConfigRepository:
    ...
    def get(self, obj_id: str, hotel_id: str = DEFAULT_HOTEL) -> Optional[WebhookConfig]:
        return (
            self.db.query(WebhookConfig)
            .filter(WebhookConfig.id == obj_id, WebhookConfig.hotel_id == hotel_id)
            .first()
        )
    ...
    def list(
        ...
        return (
            self.db.query(WebhookConfig)
            .filter(WebhookConfig.hotel_id == hotel_id)
