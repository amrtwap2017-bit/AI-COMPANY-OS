...
from datetime import datetime
"""
PurchaseOrder repository — Triangle Black
"""
import uuid
from datetime import datetime
...
class PurchaseOrderRepository:
    ...
    def create(self, data: dict) -> PurchaseOrder:
        ...
        if "po_number" not in data:
            ...
            data["po_number"] = f"{prefix}{str(count + 1).zfill(4)}"
    ...
    def get(self, obj_id: str, hotel_id: str = DEFAULT_HOTEL) -> Optional[PurchaseOrder]:
        return (
            self.db.query(PurchaseOrder)
            .filter(PurchaseOrder.id == obj_id, PurchaseOrder.hotel_id == hotel_id)
            .first()
        )

    def list(self, skip: int = 0, limit: int = 100,
             hotel_id: str = DEFAULT_HOTEL) -> List[PurchaseOrder]:
        return (
            self.db.query(PurchaseOrder)
    ...
    def update(self, obj_id: str, data: dict,
               hotel_id: str = DEFAULT_HOTEL) -> Optional[PurchaseOrder]:
