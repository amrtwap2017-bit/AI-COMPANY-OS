...
from datetime import datetime
"""
Asset repository — Triangle Black
"""
import uuid
from datetime import datetime
...
class AssetRepository:
    ...
    def create(self, data: dict) -> Asset:
        ...
        if "work_order_number" in Asset.__table__.columns.keys() and "work_order_number" not in data:
            now = datetime.utcnow()
            prefix = f"TB-WO-{now.strftime('%Y%m')}-"
            count = self.db.query(Asset).filter(
                Asset.work_order_number.like(f"{prefix}%")
            ).count()
    ...
    def get(self, obj_id: str, hotel_id: str = DEFAULT_HOTEL) -> Optional[Asset]:
        return (
            self.db.query(Asset)
            .filter(Asset.id == obj_id, Asset.hotel_id == hotel_id)
            .first()
        )

    def list(self, skip: int = 0, limit: int = 100,
             hotel_id: str = DEFAULT_HOTEL) -> List[Asset]:
        return (
            self.db.query(Asset)
            .filter(Asset.hotel_id == hotel_id)
            .order_by(Asset.created_at.desc())
            .offset(skip).limit(limit).all()
        )

    def update(self, obj_id: str, data: dict,
               hotel_id: str = DEFAULT_HOTEL) -> Optional[Asset]:
