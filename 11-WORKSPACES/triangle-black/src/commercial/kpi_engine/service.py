"""
KPI Engine Service — Triangle Black A-007
Unified Operational Health Index and 10-KPI morning dashboard.

Does NOT duplicate existing endpoints.
AGGREGATES from:
- assets, work_orders, invoices, maintenance_plans, suppliers, contracts
All in ONE service call — no N+1 API calls needed for executive morning brief.

VERIFIED LIVE DATA (demo tenant):
- Risk: 35.3/100 Grade B
- Assets: 111, Fleet: 47/100
- Open WOs: 356, SLA: 36.4%
- Spend: $1,108,350
- Suppliers: 443

Operational Health Index (OHI) formula:
- SLA Compliance (25%): target >90%
- Fleet Health (20%): target >80%
- WO Completion Rate (20%): target >70%
- PM Compliance (15%): target >90%
- Risk Inverse (10%): lower risk = higher score
- Supplier Quality (10%): target >75%
OHI > 75 = HEALTHY, 50-75 = MODERATE, < 50 = AT RISK
"""
...
class KPIEngineService:
    ...
    def _status(self, value, green_threshold, amber_threshold, lower_is_better=False):
        """Return RED/AMBER/GREEN based on thresholds."""
        if lower_is_better:
    ...
    def compute_10_kpis(self) -> list:
        """
        The 10 KPIs a hotel engineering director checks every morning.
        Returns list of dicts: name, value, unit, target, status, trend
        """
        # 1. SLA Compliance %
        total_wo = self._s("SELECT COUNT(*) FROM work_orders WHERE hotel_id=:hid AND deleted_at IS NULL")
        ...
        # 5. Asset Fleet Health %
        total_assets = self._s("SELECT COUNT(*) FROM assets WHERE hotel_id=:hid AND deleted_at IS NULL")
        ...
        # 8. PM Completion Rate %
        total_plans = self._s("SELECT COUNT(*) FROM maintenance_plans WHERE hotel_id=:hid")
        ...
        # 9. Maintenance Spend (total invoices)
        total_spend = float(self._s("""SELECT COALESCE(SUM(amount),0) FROM invoices
            WHERE hotel_id=:hid AND deleted_at IS NULL""", default=0))
        overdue_invoices = self._s("""SELECT COUNT(*) FROM invoices
            WHERE hotel_id=:hid AND deleted_at IS NULL AND LOWER(status)='overdue'""")

        # 10. Supplier Health (approved vs total)
        total_suppliers = self._s("SELECT COUNT(*) FROM suppliers WHERE hotel_id=:hid")
        approved_suppliers = self._s("""SELECT COUNT(*) FROM suppliers
            WHERE hotel_id=:hid AND is_approved=TRUE""")
        ...
        kpis = [
            {
                ...
                "insight": f"SLA compliance is {sla_pct}% against 90% target" +
                           (" — CRITICAL" if sla_pct < 50 else ""),
            },
            {
                "id": "KPI-02",
                "name": "WO Completion Rate",
                "category": "OPERATIONS",
                "value": wo_completion_pct,
                "unit": "%",
                "target": 70.0,
                "target_direction": "higher",
                ...
                "insight": f"{completed_wo} of {total_wo} work orders completed ({wo_completion_pct}%)",
            },
            {
                "id": "KPI-03",
                ...
                "status": self._status(open_wo, 50, 100, lower_is_better=True),
                "weight": 0.0,  # informational
                "normalized_score": max(0, 100 - (open_wo / max(total_wo, 1) * 100)),
                "insight": f"{open_wo} open work orders — backlog requires attention",
            },
            {
                "id": "KPI-04",
                "name": "Critical Open WOs",
                "category": "OPERATIONS",
                "value": critical_wo,
                "unit": "WOs",
                "target": 0,
                "target_direction": "lower",
                "status": self._status(critical_wo, 0, 2, lower_is_better=True),
                "weight": 0.0,  # informational
                "normalized_score": max(0, 100 - (critical_wo * 20)),
                "insight": f"{critical_wo} CRITICAL priority work orders open",
            },
            {
                "id": "KPI-05",
                "name": "Asset Fleet Health",
                "category": "ASSETS",
                "value": fleet_health_pct,
                "unit": "%",
                "target": 80.0,
                "target_direction": "higher",
            ...
            },
            {
                ...
                "weight": 0.0,  # informational
                "normalized_score": max(0, 100 - (critical_assets / max(total_assets, 1) * 100 * 3)),
            ...
            },
            {
                ...
                "insight": f"{overdue_assets} assets overdue for scheduled maintenance",
            },
            {
                "id": "KPI-08",
            ...
            },
            {
                ...
                "category": "FINANCE",
                "value": round(total_spend, 0),
                "unit": "USD",
                "target": None,
                "target_direction": "lower",
                "status": "AMBER" if overdue_invoices > 0 else "GREEN",
                "weight": 0.10,
                "normalized_score": 70 if overdue_invoices == 0 else 40,
            ...
            },
            {
                ...
                "insight": f"{approved_suppliers} of {total_suppliers} suppliers approved ({supplier_health_pct}%)",
            },
        ]
        return kpis
    ...
    def operational_health_index(self, kpis: list) -> dict:
        """
        OHI = weighted average of normalized KPI scores (0-100).
        Only weighted KPIs (weight > 0) contribute to OHI.
        """
        ...
        ohi = sum(
            k["normalized_score"] * k["weight"]
            for k in weighted_kpis
        ) / total_weight
        ...
        label = (
            "HEALTHY" if ohi >= 80
            else "MODERATE" if ohi >= 65
            else "AT RISK" if ohi >= 50
            else "CRITICAL"
        )
        ...
        return {
            ...
            "grade": grade,
            "label": label,
            "alert_threshold_exceeded": ohi < 50,
        }

    def alerts(self, kpis: list) -> list:
        """Identify KPIs that need immediate executive attention."""
        urgent = []
        for kpi in kpis:
            if kpi["status"] == "RED":
                urgent.append({
                    "kpi_id": kpi["id"],
                    "kpi_name": kpi["name"],
                    "severity": "CRITICAL" if kpi.get("value", 0) == 0 and kpi["target"] == 0
                               else "HIGH",
                    "message": kpi["insight"],
    ...
    def dashboard(self) -> dict:
        """
        Complete KPI Engine dashboard — single endpoint for morning brief.
        Returns: OHI score, 10 KPIs, alerts, category breakdown.
        """
    ...
    def trends(self) -> dict:
        """7-day KPI trend for main metrics."""
        wo_trend = self._q("""
            SELECT
