...
- Name: 09-user-journeys.spec.ts >> User Journeys — UI >> journey: work orders page shows New Work Order button
- Location: e2e/09-user-journeys.spec.ts:26:7
...
Locator: locator('button').filter({ hasText: '+ New Work Order' }).first()
Expected: visible
Timeout: 10000ms
...
Call log:
  ...
  - waiting for locator('button').filter({ hasText: '+ New Work Order' }).first()
