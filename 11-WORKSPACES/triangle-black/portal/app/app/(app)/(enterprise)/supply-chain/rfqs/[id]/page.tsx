...
const FIELDS = [
        { label: "RFQ #", key: "rfq_number" },
        { label: "Title", key: "title" },
        { label: "Budget", key: "budget_estimate" },
        { label: "Status", key: "status" },
        { label: "Deadline", key: "submission_deadline" },
...
export default function DetailPage() {
  ...
  const { data, isLoading, isError } = useQuery({
    queryKey:  ["-api-v1-rfqs", id],
    queryFn:   () => authFetchJSON("/api/v1/rfqs/" + id),
    enabled:   !!id,
    staleTime: 30_000,
    retry:     1,
  });
  ...
  if (isError || !data) {
    return (
      <PageWrapper>
        ...
        <Link href="/supply-chain/rfqs" className="flex items-center gap-2 text-sm text-amber-600 mt-4">
