...
export default function LoginPage() {
  ...
  const [email, setEmail] = useState("info@arctic-hvac.com");
    ...
    <div className="min-h-screen flex items-center justify-center relative"
      ...
      <div className="w-full max-w-md px-5 relative z-10">
        <div className="text-center mb-10">
          ...
          <p className="text-sm mt-1.5" style={{color:"#6D5F53"}}>Vendor Access</p>
        </div>

        <div className="rounded-2xl p-8" style={{background:"#2D2723",border:"1px solid #3D352F",boxShadow:"0 20px 40px rgba(0,0,0,0.35)"}}>
          <h2 className="text-lg font-bold mb-1" className="text-primary">Supplier Portal</h2>
          ...
          <form onSubmit={handleLogin} className="flex flex-col gap-4">
            ...
            <div>
              <label className="block text-xs mb-1.5 font-semibold uppercase tracking-wider" style={{color:"#B29F8B"}}>PIN Code</label>
              <input type="password" value={pin} onChange={(e: any) =>setPin(e.target.value.replace(/\D/g,"").slice(0,4))}
                maxLength={4} inputMode="numeric" required
                className="w-full rounded-md py-3 text-xl font-bold text-center tracking-widest outline-none transition-colors"
                style={{background:"#221E1B",border:"1px solid #2D2723",color:"#F3EFE8"}}
                onFocus={(e: any) =>e.target.style.borderColor="#B9924C"} onBlur={(e: any) =>e.target.style.borderColor="#2D2723"}
                placeholder="• • • •" />
            </div>
          ...
          <div className="mt-6 p-3.5 rounded-lg text-xs" style={{background:"#221E1B",color:"#6D5F53"}}>
            <div className="font-semibold mb-1 uppercase tracking-wider text-xs" style={{color:"#B29F8B"}}>Quick Access</div>
            <div>info@arctic-hvac.com</div>
          </div>
        </div>
