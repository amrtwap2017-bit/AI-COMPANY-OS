  ...
  useEffect(() => {
    ...
    if (sup?.vendor_id) {
      fetch(`/api/v1/supplier/rfqs?vendor_id=${sup.vendor_id}`, {headers:{Authorization:`Bearer ${t}`}})
        .then(r=>r.json()).then(d=>{ setRfqs(Array.isArray(d)?d:[]); setLoading(false); });
    }
  }, []);
  ...
  return (
    <div className="min-h-screen" style={{background:"#0A0F1E"}}>
      ...
      <div className="max-w-4xl mx-auto px-6 py-6">
        ...
        {loading ? <div className="space-y-3">{[1,2,3].map(i=><div key={i} className="h-24 rounded-2xl animate-pulse" style={{background:"#1E293B"}}/>)}</div>
        : rfqs.length===0 ? (
