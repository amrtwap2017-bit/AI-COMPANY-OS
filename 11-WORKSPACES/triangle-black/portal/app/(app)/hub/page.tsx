...
export default function HubPage() {
  ...
  const { data: health } = useQuery(["hub-h"], () => authFetch("/api/v1/ai/health").then(r => (r as any).data ?? r), { staleTime:120000, refetchOnWindowFocus:false });
  const { data: twin }   = useQuery(["hub-t"], () => authFetch("/api/v1/twin/state").then(r => (r as any).data ?? r), { staleTime:60000, refetchOnWindowFocus:false });
  const { data: sigRaw } = useQuery(["hub-s"], () => authFetch("/api/v1/ai/signals").then(r => (r as any).data ?? r), { staleTime:60000, refetchOnWindowFocus:false });
