...
export default function AlertsPage() {
  ...
  const { data: breaches } = useQuery(["alerts-breaches"], ()=>authFetch("/api/v1/sla/breaches").then(r => (r as any).data ?? r), {staleTime:30000});
  const { data: notifs }   = useQuery(["alerts-notifs"],   ()=>authFetch("/api/v1/platform-notif/?limit=20").then(r => (r as any).data ?? r), {staleTime:30000});
  const { data: slaDash }  = useQuery(["alerts-sla"],      ()=>authFetch("/api/v1/sla/dashboard").then(r => (r as any).data ?? r), {staleTime:60000});
