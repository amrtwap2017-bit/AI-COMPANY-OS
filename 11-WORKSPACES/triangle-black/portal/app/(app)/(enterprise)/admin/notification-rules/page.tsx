...
export default function Page() {
  ...
  const columns = [
    { key:"title", label:"Rule", render:(r:any)=>(<span className="text-sm text-slate-700">{String(r["title"]??"—")}</span>) },
    ...
    { key:"trigger", label:"Trigger", render:(r:any)=>(<span className="text-sm text-slate-700">{String(r["trigger"]??"—")}</span>) },
  ];
  return (
    <PageWrapper>
      <Breadcrumb/>
      <PageHeader title="Notification Rules" subtitle={`${items.length} records`} badge="CFG"
