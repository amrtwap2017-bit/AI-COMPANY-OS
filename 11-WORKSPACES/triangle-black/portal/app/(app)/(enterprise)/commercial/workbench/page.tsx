...
export default function CommercialWorkbenchPage() {
  ...
  const { data: finDash } = useQuery(["cw-fin"], () => authFetch("/api/v1/financial/dashboard").then(r => (r as any).data ?? r), {staleTime:60000});
  const { data: contracts } = useQuery(["cw-contracts"], () => authFetch("/api/v1/contracts/").then(r => (r as any).data ?? r), {staleTime:60000});
  const { data: leads } = useQuery(["cw-leads"], () => authFetch("/api/v1/leads/").then(r => (r as any).data ?? r), {staleTime:60000});
