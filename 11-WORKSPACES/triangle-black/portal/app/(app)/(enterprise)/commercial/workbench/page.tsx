...
export default function Page() {
  const { data, isLoading, isError, error, refetch, isFetching } = useQuery({
    queryKey: ["commercial-workbench"],
    queryFn:  () => authFetchJSON("/api/v1/actions/leads/search"),
  ...
  const columns = [
    { key:"company_name", label:"Company", render:(r:any)=>(<span className="text-sm text-slate-700">{String(r["company_name"]??"—")}</span>) },
    { key:"status", label:"Stage", render:(r:any)=>(<span className="text-sm text-slate-700">{String(r["status"]??"—")}</span>) },
    { key:"contact_name", label:"Contact", render:(r:any)=>(<span className="text-sm text-slate-700">{String(r["contact_name"]??"—")}</span>) },
    { key:"created_at", label:"Date", render:(r:any)=>(<span className="text-sm text-slate-700">{String(r["created_at"]??"—")}</span>) },
  ];
  return (
    <PageWrapper>
      <Breadcrumb/>
      <PageHeader title="Commercial Workbench" subtitle={`${items.length} records`} badge="WB"
