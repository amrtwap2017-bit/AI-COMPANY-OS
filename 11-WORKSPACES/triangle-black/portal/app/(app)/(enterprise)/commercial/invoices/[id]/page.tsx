...
export default function InvoiceDetailPage() {
  ...
  const { id } = useParams();
  const { data: inv, isLoading } = useQuery(["inv-detail",id], ()=>authFetch(`/api/v1/supplier-invoices/${id}`).then(r => (r as any).data ?? r), {enabled:!!id});
  const approve = useMutation({
    mutationFn: ()=>authFetch(`/api/v1/supplier-invoices/${id}/approve`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({action:"approve"})}).then(r => (r as any).data ?? r),
    onSuccess: ()=>{ toast.success("Invoice approved"); window.location.reload(); },
  });
  const pay = useMutation({
    mutationFn: ()=>authFetch(`/api/v1/supplier-invoices/${id}/pay`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({amount:inv?.total_amount,payment_method:"bank_transfer"})}).then(r => (r as any).data ?? r),
