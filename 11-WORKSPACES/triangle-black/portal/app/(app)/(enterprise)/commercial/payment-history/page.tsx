...
export default function PaymentHistoryPage() {
  ...
  const { data: raw=[], isLoading } = useQuery(
    ["invoices-page"],
    () => authFetch("/api/v1/invoices/?limit=100").then(r=>r.json()),
  ...
  const filtered = items.filter(x => {
    ...
    if (q && !String(x.id||"").toLowerCase().includes(q.toLowerCase())) return false;
    return true;
  });
  ...
  return (
    <PageWrapper>
      <PageHeader
        ...
        subtitle={`${items.length} invoices records`}
        breadcrumbs={[{label:"Commercial",href:"/commercial"},{label:"Payment History"}]}
      />

      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-5">
        {[{label:"Total",value:items.length,color:"text-slate-800"},{label:"Open",value:items.filter(x=>x.status==="open").length,color:"text-blue-700"},{label:"Active",value:items.filter(x=>["open","in_progress","active"].includes(x.status)).length,color:"text-indigo-700"},{label:"Completed",value:items.filter(x=>["completed","paid","received"].includes(x.status)).length,color:"text-emerald-700"}].map(k=>(
      ...
      <SectionCard title={`Invoices (${filtered.length})`}>
        <div className="flex flex-wrap items-center gap-2 mb-4 pb-4 border-b border-slate-100">
          <input type="text" placeholder="Search payment history…" value={q}
        ...
        {isLoading?<LoadingState/>:filtered.length===0?<EmptyState title="No invoices found" subtitle="Adjust filters or check your data"/>:(
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead><tr className="border-b border-slate-100">
                <th className="text-left py-2 px-3 text-xs font-semibold text-secondary uppercase tracking-wide whitespace-nowrap">Title/Name</th> <th className="text-left py-2 px-3 text-xs font-semibold text-secondary uppercase tracking-wide whitespace-nowrap">Status</th> <th className="text-left py-2 px-3 text-xs font-semibold text-secondary uppercase tracking-wide whitespace-nowrap">Amount</th> <th className="text-left py-2 px-3 text-xs font-semibold text-secondary uppercase tracking-wide whitespace-nowrap">Date</th>
              </tr></thead>
              <tbody className="divide-y divide-slate-50">
                {filtered.map(x=>(
                  <tr key={x.id} className="hover:bg-slate-50 transition-colors">
                    <td className="py-3 px-3"><p className="font-medium text-slate-800 truncate max-w-xs">{x.id||"—"}</p>{x.description&&<p className="text-xs text-tertiary truncate">{x.description?.slice(0,60)}</p>}</td>
                    ...
                    <td className="py-3 px-3"><span className="font-semibold text-slate-800">EGP {fmtNum(x.total_amount)}</span></td>
