    ...
    <div className="p-6 space-y-6 bg-slate-50 dark:bg-slate-950 min-h-screen">
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <h1 className="text-3xl font-black text-slate-900 dark:text-white">Lead Pipeline</h1>
          <p className="text-slate-500 text-sm mt-1.5">{leads.length} total · {activeLeads.length} active · {hotLeads.length} hot leads</p>
        </div>
      ...
      <div className="flex gap-3 flex-wrap">
        ...
        <div className="text-xs text-slate-400 self-center">{filtered.length} leads · avg score {avgScore}</div>
      </div>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 overflow-hidden">
        {filtered.length === 0 ? (
          <div className="text-center py-16">
            ...
            <div className="font-bold text-slate-900 dark:text-white text-lg">No leads found</div>
            <button onClick={()=>router.push("/leads/new")}
          ...
          <>
            <div className="grid grid-cols-[1fr_140px_100px_80px_100px] bg-slate-50 dark:bg-slate-800/50 px-5 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wider">
              <div>Lead</div>
              <div>Company</div>
              ...
              {filtered.map((l,i)=>{
                ...
                return (
                  <button key={i} onClick={()=>router.push(`/commercial/leads/${l.id}`)}
                    className="w-full grid grid-cols-[1fr_140px_100px_80px_100px] items-center px-5 py-4 text-left hover:bg-amber-50/50 dark:hover:bg-amber-900/10 transition-colors group">
                    <div className="min-w-0 pr-4">
                      <div className="flex items-center gap-2">
                        <div className="font-semibold text-sm text-slate-900 dark:text-white truncate group-hover:text-amber-600">{l.name}</div>
                        {isHot && <span className="text-[10px] bg-red-100 text-red-700 px-1.5 py-0.5 rounded font-bold flex-shrink-0">🔥 HOT</span>}
                      </div>
                      <div className="text-xs text-slate-400 mt-0.5">{l.source || "—"} · {l.email || "—"}</div>
                    </div>
                    <div className="text-xs text-slate-500 truncate">{l.company || "—"}</div>
                    <div className="text-center">
                      <span className={`text-xs font-medium px-2 py-0.5 rounded-lg ${STATUS_BADGE[l.status] || "bg-slate-100 text-slate-600"}`}>
                        {l.status || "—"}
                      </span>
                    </div>
                    <div className="text-center">
                      <div className={`text-sm font-black ${score>=70?"text-emerald-500":score>=50?"text-amber-500":"text-slate-500"}`}>{score}</div>
                    ...
                    <div className="text-center text-xs text-slate-400">{fmtDate(l.updated_at)}</div>
                  </button>
                );
              })}
            </div>
