...
export default function ProfilePage() {
  ...
  const { data: me } = useQuery({ queryKey:["profile-me"], queryFn:()=>authFetch("/api/v1/me").then(r => (r as any).data ?? r), staleTime:60000 });
  ...
  const changePwdMut = useMutation({
    mutationFn: (payload) => authFetch("/api/v1/secure/auth/change-password",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(payload)}).then(r => (r as any).data ?? r),
    onSuccess: (data) => {
