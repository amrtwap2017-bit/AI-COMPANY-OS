...
export default function PlatformMaturityPage() {
  const { data: twin } = useQuery(["mat-twin"], () => authFetch("/api/v1/twin/state").then(r => (r as any).data ?? r), {staleTime:30000});
  const { data: health } = useQuery(["mat-health"], () => authFetch("/api/v1/health").then(r => (r as any).data ?? r), {staleTime:30000});
