...
export default function PlatformHealthPage() {
  ...
  const { data: health } = useQuery({ queryKey:["platform-health"], queryFn:()=>fetch("/api/v1/health/detailed").then(r => (r as any).data ?? r), staleTime:30000, refetchInterval:60000 });
  const { data: summary, isLoading } = useQuery({ queryKey:["platform-summary"], queryFn:()=>authFetch("/api/v1/platform/summary").then(r => (r as any).data ?? r), staleTime:30000 });
  const { data: tenant } = useQuery({ queryKey:["tenant-current"], queryFn:()=>fetch("/api/v1/tenants/current").then(r => (r as any).data ?? r), staleTime:60000 });
  const { data: auditRaw } = useQuery({ queryKey:["platform-audit-recent"], queryFn:()=>authFetch("/api/v1/security/audit").then(r => (r as any).data ?? r), staleTime:30000 });
