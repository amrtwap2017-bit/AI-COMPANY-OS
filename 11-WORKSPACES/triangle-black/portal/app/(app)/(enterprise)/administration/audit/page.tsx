...
export default function Page() {
  ...
  const columns = [
    { key:"user", label:"User", render:(r:any)=>(<span className="text-sm text-slate-700">{String(r["user"]??"—")}</span>) },
    ...
    { key:"timestamp", label:"Time", render:(r:any)=>(<span className="text-sm text-slate-700">{String(r["timestamp"]??"—")}</span>) },
  ];
  return (
    <PageWrapper>
      <Breadcrumb/>
      <PageHeader title="Audit Trail" subtitle={`${items.length} records`} badge="AUD"
