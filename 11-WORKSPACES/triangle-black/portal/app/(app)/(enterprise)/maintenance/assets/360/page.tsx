...
export default function Page() {
  const { data, isLoading, isError, error, refetch, isFetching } = useQuery({
    queryKey: ["maintenance-assets-360"],
  ...
  const columns = [
    ...
    { key:"serial_number", label:"Serial", render:(r:any)=>(<span className="text-sm text-slate-700">{String(r["serial_number"]??"—")}</span>) },
    { key:"last_maintenance", label:"Last PM", render:(r:any)=>(<span className="text-sm text-slate-700">{String(r["last_maintenance"]??"—")}</span>) },
    { key:"health", label:"Health", render:(r:any)=>(<span className="text-sm text-slate-700">{String(r["health"]??"—")}</span>) },
  ];
  return (
    <PageWrapper>
      <Breadcrumb/>
      <PageHeader title="Asset 360°" subtitle={`${items.length} records`} badge="360"
