    ...
    <div className="p-6 space-y-6 bg-slate-50 dark:bg-slate-950 min-h-screen">
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <h1 className="text-3xl font-black text-slate-900 dark:text-white">Asset Registry</h1>
          <p className="text-slate-500 text-sm mt-1.5">{assets.length} assets · {operational.length} operational · {critical.length} critical</p>
        </div>
      ...
      <div className="flex gap-3 flex-wrap">
        ...
        {(search||catFilter!=="all"||critFilter!=="all") && (
          <button onClick={()=>{setSearch("");setCatFilter("all");setCritFilter("all");}}
            className="px-3 py-2 text-xs text-slate-500 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl hover:text-slate-700">
            Clear ×
          </button>
        )}
        <div className="text-xs text-slate-400 self-center">{filtered.length} assets</div>
      </div>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 overflow-hidden">
        {filtered.length === 0 ? (
          <div className="text-center py-16">
            ...
            <div className="font-bold text-slate-900 dark:text-white text-lg">No assets found</div>
            <div className="text-slate-400 text-sm mt-1">Try adjusting your search or filters</div>
          </div>
        ) : (
          <>
            <div className="grid grid-cols-[1fr_120px_100px_100px_120px_120px] bg-slate-50 dark:bg-slate-800/50 px-5 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wider">
              <div>Asset</div>
              <div>Category</div>
            ...
            <div className="divide-y divide-slate-50 dark:divide-slate-800">
              {filtered.map((a,i)=>{
                const sc = STATUS_CONFIG[a.status] || { cls:"bg-slate-100 text-slate-600", dot:"bg-slate-400" };
                ...
                return (
                  <button key={i} onClick={()=>router.push(`/maintenance/assets/${a.id}`)}
                    className="w-full grid grid-cols-[1fr_120px_100px_100px_120px_120px] items-center px-5 py-4 text-left hover:bg-amber-50/50 dark:hover:bg-amber-900/10 transition-colors group">
                    <div className="min-w-0 pr-4">
                      <div className="font-semibold text-sm text-slate-900 dark:text-white truncate group-hover:text-amber-600 transition-colors">{a.name}</div>
                      <div className="text-xs text-slate-400 mt-0.5">{a.manufacturer} {a.model} · {a.location_description || a.serial_number || "—"}</div>
                    </div>
                    ...
                    <div className="text-center">
                      <span className={`text-xs font-medium px-2 py-0.5 rounded-lg ${CRIT_CONFIG[a.criticality] || "bg-slate-100 text-slate-600"}`}>
                        {a.criticality || "—"}
                      </span>
                    </div>
                    <div className="text-center text-xs text-slate-500">{fmtDate(a.last_maintenance_date)}</div>
