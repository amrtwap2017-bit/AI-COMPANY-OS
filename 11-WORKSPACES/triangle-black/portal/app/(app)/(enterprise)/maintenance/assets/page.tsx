...
export default function MaintenanceAssetsPage() {
  ...
  const { query, setQuery, filtered } = useSearch(data, ["name","category","serial_number","location_description"]);
  const { page, totalPages, items, goToPage } = usePagination(filtered, pageSize);
  const tabs = STATUS_TABS.map(t => ({ ...t, count: t.value === "all" ? data.length : data.filter((a:any) => a.status === t.value).length }));

  const columns = [
    { key: "name", label: "Asset", sortable: true,
      render: (row:any) => (<div><p className="font-semibold text-sm text-slate-900">{row.name}</p><p className="text-xs text-slate-400">{row.serial_number || "—"}</p></div>) },
    { key: "category", label: "Category",
      render: (row:any) => <span className="text-xs bg-blue-50 text-blue-700 px-2 py-0.5 rounded-md font-medium">{row.category || "—"}</span> },
  ...
  return (
    <PageWrapper>
      ...
      <ActionBar search={{ value: query, onChange: setQuery, placeholder: "Search assets..." }} resultCount={filtered.length} totalCount={data.length} />
