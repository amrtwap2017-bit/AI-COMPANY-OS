      ...
      <div className="tb-hero">
        <div className="tb-hero-inner">
          ...
          <div className="grid mt-6" style={{gridTemplateColumns:"repeat(6,1fr)",gap:12}}>
            {[{label:"Total",value:assets.length},{label:"Operational",value:operational.length,good:true},{label:"In Fault",value:faulted.length,danger:faulted.length>0},{label:"Maintenance",value:underMaint.length},{label:"Critical",value:critical.length,danger:critical.length>0},{label:"Overdue Svc",value:overdueService.length,danger:overdueService.length>0}].map((k,i)=>(
      ...
      <div className="tb-canvas">
        {faulted.length>0 && (
          <div className="tb-alert tb-alert-danger mb-4">
            ...
            <div className="flex-1 text-sm">{faulted.length} Asset{faulted.length>1?"s":""} In Fault — {faulted.slice(0,2).map(a=>a.name).join(" · ")}</div>
            <button onClick={()=>router.push("/maintenance/actions")} className="tb-btn tb-btn-danger tb-btn-sm">View Faults →</button>
          </div>
        )}

        <div className="flex gap-2.5 flex-wrap items-center mb-4">
          <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search assets by name or serial..." className="tb-input" style={{maxWidth:"320px"}} />
          <select value={catF} onChange={e=>setCatF(e.target.value)} className="tb-select" style={{width:"auto"}}>
            <option value="all">All Categories</option>
            {cats.map(c=><option key={c} value={c}>{c}</option>)}
          </select>
          <select value={critF} onChange={e=>setCritF(e.target.value)} className="tb-select" style={{width:"auto"}}>
              ...
              <table className="tb-table">
                ...
                <tbody>
                  {filtered.map((a,i)=>{
                    ...
                    const cc = CRIT_COLOR[a.criticality]||"var(--color-text-3)";
                    return (
