...
const S = {Operational:"bg-emerald-100 text-emerald-800","Under Maintenance":"bg-amber-100 text-amber-800","In Fault":"bg-red-100 text-red-800",Decommissioned:"bg-slate-100 text-secondary"};
const C = {critical:"bg-red-100 text-red-800 border-red-200",high:"bg-orange-100 text-orange-800 border-orange-200",medium:"bg-amber-100 text-amber-800 border-amber-200",low:"bg-slate-100 text-secondary border-slate-200"};
  ...
  if (!asset || asset.detail) return (
    <PageWrapper>
      <div className="text-center py-20">
        <p className="text-secondary mb-4">Asset not found</p>
    ...
    <PageWrapper>
      ...
      {!editing && (
        <div className="flex items-center gap-3 mb-5 p-4 bg-white border border-slate-200 rounded-xl">
          ...
          <span className={"inline-flex items-center px-3 py-1 rounded-full text-sm font-semibold "+(S[asset.status]||"bg-slate-100 text-secondary")}>{asset.status||"—"}</span>
          <div className="flex-1" />
          <span className="text-xs text-tertiary mr-1">Change to:</span>
            ...
            {editing ? (
              <form onSubmit={save} className="space-y-4">
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block text-xs font-semibold text-secondary mb-1">Asset Name *</label>
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div>
                    <label className="block text-xs font-semibold text-secondary mb-1">Criticality</label>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block text-xs font-semibold text-secondary mb-1">Location</label>
                    <input value={form?.location||""} onChange={e=>setForm({...form,location:e.target.value})} placeholder="e.g. B2 Plant Room" className={inp} />
                  </div>
                  <div>
                    <label className="block text-xs font-semibold text-secondary mb-1">Model / Manufacturer</label>
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <p className="text-xs font-semibold text-secondary uppercase tracking-wide mb-1">Category</p>
                  ...
                  <div>
                    <p className="text-xs font-semibold text-secondary uppercase tracking-wide mb-1">Model</p>
                    <p className="text-slate-700">{asset.model||asset.manufacturer||"—"}</p>
                  </div>
                  <div>
                    <p className="text-xs font-semibold text-secondary uppercase tracking-wide mb-1">Serial Number</p>
        ...
        <div className="space-y-4">
          <SectionCard title="Properties">
            <dl className="space-y-3">
              {[
                {label:"Status",      value:<span className={"inline-flex items-center px-2 py-0.5 rounded-full text-xs font-semibold "+(S[asset.status]||"bg-slate-100 text-secondary")}>{asset.status||"—"}</span>},
          ...
          <SectionCard title="Quick Actions">
            <div className="space-y-2">
              ...
              <Link href="/maintenance/assets" className="block w-full px-3 py-2 text-sm font-semibold text-secondary bg-slate-50 border border-slate-200 rounded-xl hover:bg-slate-100 text-center">Back to Assets</Link>
            </div>
          </SectionCard>
        </div>
      </div>
