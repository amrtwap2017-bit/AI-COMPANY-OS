...
export default function AssetTreePage() {
  ...
  const { data: assetRaw, isLoading } = useQuery(
    ["tree-assets"],
    () => authFetch("/api/v1/assets/").then(r => r.data ?? r),
    { refetchInterval: 60000 }
  );
  const { data: woRaw } = useQuery(["tree-wos"], () => authFetch("/api/v1/work-orders/").then(r => r.data ?? r));
  const { data: pmRaw } = useQuery(["tree-pms"], () => authFetch("/api/v1/maintenance/pm-plans/").then(r => r.data ?? r));
