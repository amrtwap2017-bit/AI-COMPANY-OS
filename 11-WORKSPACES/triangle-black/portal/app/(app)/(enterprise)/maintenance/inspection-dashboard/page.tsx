...
export default function InspectionDashboardPage() {
  ...
  const { data: rawPlans, isLoading } = useQuery({ queryKey:["pm-plans-inspection"], queryFn:()=>authFetch("/api/v1/maintenance/pm-plans/").then(r => (r as any).data ?? r), staleTime:60000 });
  const { data: stats } = useQuery({ queryKey:["pm-stats-insp"], queryFn:()=>authFetch("/api/v1/pm-schedule/stats").then(r => (r as any).data ?? r), staleTime:60000 });
