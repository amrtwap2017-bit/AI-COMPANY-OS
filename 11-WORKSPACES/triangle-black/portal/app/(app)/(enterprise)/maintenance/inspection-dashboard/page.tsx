  ...
  return (
    <PageWrapper>
      <PageHeader
        ...
        subtitle={`${items.length} total maintenance records`}
      ...
      <SectionCard title={`Maintenance (${filtered.length})`}>
        <div className="flex flex-wrap items-center gap-2 mb-4 pb-4 border-b border-slate-100">
          <input type="text" placeholder="Search inspection dashboard…" value={q} onChange={e=>setQ(e.target.value)}
            className="border border-slate-200 rounded-lg px-3 py-1.5 text-sm w-52 focus:outline-none focus:border-blue-400" />
          <select value={sf} onChange={e=>setSf(e.target.value)} className="border border-slate-200 rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:border-blue-400"><option value="all">All Status</option><option value="open">open</option><option value="in_progress">in progress</option><option value="completed">completed</option><option value="active">active</option><option value="paid">paid</option></select>
          {(sf!=="all"||q)&&<button onClick={()=>{setSf("all");setQ("")}} className="text-xs text-tertiary hover:text-red-500 underline">Clear</button>}
        </div>
        {isLoading?<LoadingState/>:filtered.length===0?<EmptyState title="No maintenance found" subtitle="Adjust filters or check your data"/>:(
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead><tr className="border-b border-slate-100">
                <th className="text-left py-2 px-3 text-xs font-semibold text-secondary uppercase tracking-wide whitespace-nowrap">Title/Name</th> <th className="text-left py-2 px-3 text-xs font-semibold text-secondary uppercase tracking-wide whitespace-nowrap">Status</th> <th className="text-left py-2 px-3 text-xs font-semibold text-secondary uppercase tracking-wide whitespace-nowrap">Date</th>
              </tr></thead>
              <tbody className="divide-y divide-slate-50">
                {filtered.map(x=>(
                  <tr key={x.id} className="hover:bg-slate-50 transition-colors">
                    <td className="py-3 px-3"><p className="font-medium text-slate-800 truncate max-w-xs">{x.title||"—"}</p>{x.description&&<p className="text-xs text-tertiary truncate">{x.description?.slice(0,60)}</p>}</td>
