...
export default function Page() {
  ...
  const columns = [
    { key:"name", label:"Customer", render:(r:any)=>(<span className="text-sm text-slate-700">{String(r["name"]??"—")}</span>) },
    ...
    { key:"health_score", label:"Health", render:(r:any)=>(<span className="text-sm text-slate-700">{String(r["health_score"]??"—")}</span>) },
  ];
  return (
    <PageWrapper>
      <Breadcrumb/>
      <PageHeader title="Customers" subtitle={`${items.length} records`} badge="CX"
