...
export default function CustomersPage() {
  ...
  const { data: contRaw } = useQuery(["cust-conts"], () => authFetch("/api/v1/contracts/").then(r => (r as any).data ?? r));
  const { data: leadRaw } = useQuery(["cust-leads"], () => authFetch("/api/v1/leads-portal").then(r => (r as any).data ?? r));
  const { data: invRaw }  = useQuery(["cust-inv"],   () => authFetch("/api/v1/invoices/").then(r => (r as any).data ?? r));
  const contracts = toArr(contRaw); const leads = toArr(leadRaw); const inv = toArr(invRaw);
  const clients = [...new Set(contracts.map((c: any) =>c.client_name).filter(Boolean))];
      ...
      <div className="tb-hero" style={{background:"linear-gradient(135deg, #221D1A 0%, #1A0F28 100%)"}}>
        <div className="tb-hero-inner">
          ...
          <div className="tb-grid-4 mt-6">
            {[{label:"Clients",value:clients.length,color:"#221D1A"},{label:"Active Contracts",value:activeContracts.length,color:"#547C4D"},{label:"Total Leads",value:leads.length,color:"#5B7C8C"},{label:"Revenue",value:fmtEGP(revenue),color:"#B07A2A"}].map((k: any, i: number) =>(
      ...
      <div className="tb-canvas">
        ...
        <div className="tb-section">
          ...
          <div className="tb-grid-4">
            {[{label:"Contracts",icon:"📄",path:"/commercial/contracts"},{label:"Leads",icon:"👤",path:"/commercial/leads"},{label:"Pipeline",icon:"📊",path:"/commercial/pipeline"},{label:"Invoices",icon:"💰",path:"/invoices"}].map((a: any, i: number) =>(
