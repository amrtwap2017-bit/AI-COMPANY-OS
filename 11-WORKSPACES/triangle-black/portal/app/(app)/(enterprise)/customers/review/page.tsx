...
export default function Page() {
  ...
  const columns = [
    ...
    { key:"renewal_date", label:"Renewal", render:(r:any)=>(<span className="text-sm text-slate-700">{String(r["renewal_date"]??"—")}</span>) },
    { key:"risk", label:"Risk", render:(r:any)=>(<span className="text-sm text-slate-700">{String(r["risk"]??"—")}</span>) },
  ];
  return (
    <PageWrapper>
      <Breadcrumb/>
      <PageHeader title="Customer Review" subtitle={`${items.length} records`} badge="REV"
