...
export default function AnalyticsCosts() {
  ...
  return (
    <div className="p-6 space-y-6 bg-slate-50 dark:bg-slate-950 min-h-screen">
      ...
      <h1 className="text-3xl font-black text-slate-900 dark:text-white">Cost Analysis</h1>
      <p className="text-slate-500 mt-1">Revenue, procurement spend, and financial performance</p></div>
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          ...
          <button key={i} onClick={()=>router.push(k.path)} className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-5 text-left hover:border-amber-400 hover:shadow-lg transition-all">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-6">
          <h2 className="font-bold text-slate-900 dark:text-white mb-4">Invoice Revenue Breakdown</h2>
          {[
            ...
            <div key={i} className="flex justify-between items-center py-3 border-b border-slate-50 dark:border-slate-800 last:border-0">
              <div className="flex items-center gap-2"><div className={`w-3 h-3 rounded-full bg-${s.color}-500`}/><span className="text-sm text-slate-700 dark:text-slate-300">{s.label}</span></div>
              <div className="text-right"><div className={`font-black text-${s.color}-500`}>{s.count}</div><div className="text-xs text-slate-400">{fmtEGP(s.value)}</div></div>
            </div>
          ))}
        </div>
        <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-6">
          <h2 className="font-bold text-slate-900 dark:text-white mb-4">Procurement Summary</h2>
          {[
