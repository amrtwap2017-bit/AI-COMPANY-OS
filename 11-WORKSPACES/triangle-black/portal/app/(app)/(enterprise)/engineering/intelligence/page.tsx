...
export default function EngineeringIntelligencePage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const summary = await engineeringIntelligenceApi.summary();
        if (!active) return;
        setData(summary);
      } catch (e: any) {
        if (!active) return;
    ...
    <div className="space-y-6">
      ...
      <section className="rounded-3xl border border-slate-200 bg-white p-8 shadow-sm">
        ...
        <p className="mt-3 max-w-4xl text-base leading-7 text-slate-600">
          Cross-section engineering visibility for BOQ, specifications, drawings, inspections, quality, safety, and technical records.
        </p>
      </section>
        ...
        <section className="grid gap-4 md:grid-cols-3 xl:grid-cols-4">
          {Object.entries(data.sections || {}).map(([key, value]) => (
            <div key={key} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
              <div className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">{key}</div>
              <div className="mt-2 text-3xl font-semibold tracking-tight text-slate-950">{String(value)}</div>
            </div>
          ))}
        </section>
      ) : (
