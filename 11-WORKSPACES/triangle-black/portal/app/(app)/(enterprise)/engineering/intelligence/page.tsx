        ...
        <div className="bg-white dark:bg-slate-900 rounded-2xl border p-6">
          ...
          {criticalAssets.slice(0,8).map((a,i)=>(
            <button key={i} onClick={()=>router.push(`/maintenance/assets/${a.id}`)}
              ...
              <div><div className="text-sm font-medium truncate">{a.name}</div><div className="text-xs text-secondary">{a.category}</div></div>
              <div className="text-xs text-tertiary ml-2">{fmtDate(a.last_maintenance_date)}</div>
            </button>
          ))}
        </div>
      </div>
