...
export default function OperationsWorkbenchPage() {
  ...
  const { data: rawWOs, isLoading } = useQuery({queryKey:["wb-wos"],queryFn:()=>authFetch("/api/v1/work-orders/?limit=50").then(r => r.data ?? r),staleTime:30000,refetchInterval:60000});
  const { data: dispatch } = useQuery({queryKey:["wb-dispatch"],queryFn:()=>authFetch("/api/v1/dispatch/board").then(r => r.data ?? r),staleTime:30000});
  const { data: sla } = useQuery({queryKey:["wb-sla"],queryFn:()=>authFetch("/api/v1/sla/dashboard").then(r => r.data ?? r),staleTime:60000});
