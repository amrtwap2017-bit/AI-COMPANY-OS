...
export default function ?Limit=100&Status=Open|Domain:Work_OrdersPage() {
  const [q, setQ] = useState("");
  const [sf, setSf] = useState("all");

  const { data: raw=[], isLoading } = useQuery(
    ["work_orders-list"],
    () => authFetch("/api/v1/work-orders/?limit=100&status=open|DOMAIN:work_orders").then(r=>r.json()),
  ...
  return (
    <PageWrapper>
      <PageHeader
        title="?Limit=100&Status=Open|Domain:Work_Orders"
        subtitle={`${items.length} total work orders records`}
        breadcrumbs={[{label:"Operations",href:"/operations"},, {label:"Dispatch View|Api:",href:"/operations/dispatch-view|API:"},, {label:"Api",href:"/operations/dispatch-view|API:/api"},, {label:"V1",href:"/operations/dispatch-view|API:/api/v1"},, {label:"Work Orders",href:"/operations/dispatch-view|API:/api/v1/work-orders"},, {label:"?Limit=100&Status=Open|Domain:Work_Orders"}]}
      />

      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-5">
        {[{label:"Total",value:items.length,color:"text-slate-800"}, {{label:"Open",value:items.filter(x=>x.status==="open").length,color:"text-blue-700"}}, {{label:"Completed",value:items.filter(x=>x.status==="completed").length,color:"text-emerald-700"}}, {{label:"Critical",value:items.filter(x=>x.priority==="critical"&&x.status!=="completed").length,color:"text-red-700"}}].map(k=>(
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead><tr className="border-b border-slate-100">
                <th key="Title/Name" className="text-left py-2 px-3 text-xs font-semibold text-slate-500 uppercase tracking-wide whitespace-nowrap">Title/Name</th> <th key="Priority" className="text-left py-2 px-3 text-xs font-semibold text-slate-500 uppercase tracking-wide whitespace-nowrap">Priority</th> <th key="Status" className="text-left py-2 px-3 text-xs font-semibold text-slate-500 uppercase tracking-wide whitespace-nowrap">Status</th> <th key="Date" className="text-left py-2 px-3 text-xs font-semibold text-slate-500 uppercase tracking-wide whitespace-nowrap">Date</th>
              </tr></thead>
