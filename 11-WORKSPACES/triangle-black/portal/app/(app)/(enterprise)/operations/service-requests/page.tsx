...
export default function ServiceRequestsPage() {
  ...
  const createSR = useMutation(
    (payload) => authFetch("/api/v1/service-requests/",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(payload)}).then(r => (r as any).data ?? r),
  ...
  const { data: srRaw, isLoading } = useQuery(["sr-list"], ()=>authFetch("/api/v1/service-requests/").then(r => (r as any).data ?? r), {refetchInterval:30000});
  const { data: woRaw } = useQuery(["sr-wos"], ()=>authFetch("/api/v1/work-orders/").then(r => (r as any).data ?? r));
