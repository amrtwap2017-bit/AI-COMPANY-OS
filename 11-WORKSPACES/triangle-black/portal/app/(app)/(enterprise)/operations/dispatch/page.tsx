...
export default function DispatchBoardPage() {
  ...
  const { data: boardData, isLoading } = useQuery(
    ...
    () => {
      ...
      return authFetch(`/api/v1/dispatch/board?${params}`).then(r => (r as any).data ?? r);
    },
  ...
  const assignMut = useMutation(
    ({wo_id,tech_id})=>authFetch(`/api/v1/work-orders/${wo_id}/assign`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({technician_id:tech_id})}).then(r => (r as any).data ?? r),
    {onSuccess:()=>qc.invalidateQueries(["dispatch-board"])}
  );
  const statusMut = useMutation(
    ({wo_id,status})=>authFetch(`/api/v1/work-orders/${wo_id}/status`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({status})}).then(r => (r as any).data ?? r),
    {onSuccess:()=>qc.invalidateQueries(["dispatch-board"])}
  );
