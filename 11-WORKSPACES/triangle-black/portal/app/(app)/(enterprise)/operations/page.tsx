...
export default function OperationsPage() {
  ...
  const { data: woRaw }   = useQuery(["ops-wos"],   () => authFetch("/api/v1/work-orders/").then(r => r.data ?? r));
  const { data: techRaw } = useQuery(["ops-techs"], () => authFetch("/api/v1/technicians/").then(r => r.data ?? r));
  const { data: srRaw }   = useQuery(["ops-srs"],   () => authFetch("/api/v1/service-requests/").then(r => r.data ?? r));
