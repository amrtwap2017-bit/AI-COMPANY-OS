...
export default function TechnicianDetailPage() {
  ...
  const { data: tech, isLoading } = useQuery({ queryKey:["tech-detail",id], queryFn:()=>authFetch(`/api/v1/technicians/${id}`).then(r => (r as any).data ?? r), enabled:!!id });
  const { data: rawWOs, isLoading: wosLoading } = useQuery({ queryKey:["tech-wos",id], queryFn:()=>authFetch(`/api/v1/technicians/${id}/work-orders`).then(r => (r as any).data ?? r), enabled:!!id });
