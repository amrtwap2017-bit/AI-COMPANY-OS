...
export default function AssetQRGalleryPage() {
  ...
  const { data: raw, isLoading } = useQuery({ queryKey:["asset-qr-list"], queryFn:()=>authFetch("/api/v1/qr/assets/list?limit=200").then(r=>r.json()), staleTime:60000 });
        ...
        <div className="tb-hero-inner">
          ...
          <div className="tb-grid-4 mt-6">
            {isLoading ? <KpiSkeleton /> : <>
              ...
              <div className="tb-hero-kpi"><div className="tb-hero-kpi-value" style={{color:"var(--color-danger)"}}>{critical}</div><div className="tb-hero-kpi-label">Critical</div></div>
      ...
      <div className="tb-canvas">
        <div className="tb-section" style={{padding:"16px 20px"}}>
          <div className="flex gap-2.5 flex-wrap items-center">
            <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search assets..." className="tb-input" style={{minWidth:"200px",width:"auto"}} />
        ...
        {isLoading ? (
          <div className="tb-grid-4">
            {[1,2,3,4,5,6,7,8].map(i=><div key={i} className="tb-shimmer tb-shimmer-block" style={{height:280}} />)}
          </div>
        ) : filtered.length === 0 ? (
                ...
                <div className="p-3">
                  ...
                  <div className="tb-grid-3 gap-1.5">
                    ...
                    <button onClick={()=>router.push(`/asset/${asset.id}`)}
                      className="tb-btn tb-btn-sm justify-center text-xs"
                      style={{background:"var(--color-success-bg)",color:"var(--color-success)",border:"1px solid var(--color-success-border)"}}>View</button>
                  </div>
                </div>
              </div>
            ))}
            ...
            <div className="tb-table-wrap">
              <table className="tb-table">
                ...
                <tbody>
                  {filtered.map((asset,i) => (
                    <tr key={asset.id||i}>
                      <td><img src={`/api/v1/qr/asset/${asset.id}?size=60`} alt="QR"
                        className="rounded-md border border-default bg-white" style={{width:52,height:52}} /></td>
                      <td>
