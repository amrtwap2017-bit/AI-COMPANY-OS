...
export default function BulkOperationsPage() {
  ...
  const { data: rawWOs, isLoading } = useQuery({queryKey:["bulk-wos"],queryFn:()=>authFetch("/api/v1/work-orders/?limit=100").then(r => (r as any).data ?? r),staleTime:30000});
  const { data: rawTechs } = useQuery({queryKey:["bulk-techs"],queryFn:()=>authFetch("/api/v1/technicians/").then(r => (r as any).data ?? r).catch(()=>[]),staleTime:60000});
