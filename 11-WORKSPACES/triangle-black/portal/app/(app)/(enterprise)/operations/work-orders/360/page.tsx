...
export default function Page() {
  const { data, isLoading, isError, error, refetch, isFetching } = useQuery({
    queryKey: ["operations-work-orders-360"],
  ...
  const columns = [
    ...
    { key:"technician", label:"Technician", render:(r:any)=>(<span className="text-sm text-slate-700">{String(r["technician"]??"—")}</span>) },
    { key:"due_date", label:"Due", render:(r:any)=>(<span className="text-sm text-slate-700">{String(r["due_date"]??"—")}</span>) },
  ];
  return (
    <PageWrapper>
      <Breadcrumb/>
      <PageHeader title="Work Order 360°" subtitle={`${items.length} records`} badge="360"
