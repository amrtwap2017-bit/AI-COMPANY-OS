    ...
    <PageWrapper>
      ...
      {!editing && (
        <div className="flex items-center gap-3 mb-5 p-4 bg-white border border-slate-200 rounded-xl">
          ...
          <span className={"inline-flex items-center px-3 py-1 rounded-full text-sm font-semibold "+(S[wo.status]||"bg-slate-100 text-slate-600")}>{wo.status?.replace(/_/g," ")||"—"}</span>
          ...
          <SectionCard title="Properties">
            <dl className="space-y-3">
              {[
                ...
                {label:"Type",      value:<span className="inline-flex items-center px-2 py-0.5 rounded text-xs bg-slate-100 text-slate-600">{wo.type||"—"}</span>},
