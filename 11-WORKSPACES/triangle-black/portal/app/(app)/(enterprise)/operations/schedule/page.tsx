...
export default function SchedulePage() {
  ...
  const { data: rawPlans, isLoading } = useQuery({queryKey:["schedule-plans"],queryFn:()=>authFetch("/api/v1/maintenance/pm-plans/").then(r => (r as any).data ?? r),staleTime:60000});
  const { data: stats } = useQuery({queryKey:["schedule-stats"],queryFn:()=>authFetch("/api/v1/pm-schedule/stats").then(r => (r as any).data ?? r),staleTime:60000});
