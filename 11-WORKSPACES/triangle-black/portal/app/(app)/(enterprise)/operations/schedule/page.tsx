  ...
  return (
    <PageWrapper>
      <PageHeader
        ...
        breadcrumbs={[{label:"Operations",href:"/operations"},{label:"Schedule"}]}
      />
      ...
      <SectionCard title={`Work Orders (${filtered.length})`}>
        <div className="flex flex-wrap items-center gap-2 mb-4 pb-4 border-b border-slate-100">
          <input type="text" placeholder="Search schedule…" value={q}
              ...
              <tbody className="divide-y divide-slate-50">
                {filtered.map(x=>(
                  <tr key={x.id} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="py-3 px-3"><span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-semibold border ${(x.priority==="critical"?"bg-red-100 text-red-800 border-red-200":x.priority==="high"?"bg-orange-100 text-orange-800 border-orange-200":x.priority==="medium"?"bg-amber-100 text-amber-800 border-amber-200":"bg-slate-100 text-secondary border-slate-200")}`}>{x.priority||"—"}</span></td>
