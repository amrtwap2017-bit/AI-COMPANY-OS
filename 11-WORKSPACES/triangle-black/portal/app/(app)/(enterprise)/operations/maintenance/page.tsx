...
export default function MaintenanceSchedulerPage() {
  ...
  const { data: schedData, isLoading } = useQuery(["pm-schedule-assets"],()=>authFetch("/api/v1/pm-schedule/assets?limit=100").then(r => (r as any).data ?? r),{staleTime:30000,refetchInterval:60000});
  const { data: calData } = useQuery(["pm-schedule-calendar"],()=>authFetch("/api/v1/pm-schedule/calendar").then(r => (r as any).data ?? r),{staleTime:60000});
  const { data: stats } = useQuery(["pm-schedule-stats"],()=>authFetch("/api/v1/pm-schedule/stats").then(r => (r as any).data ?? r),{staleTime:30000});

  const genMut = useMutation(
    ()=>authFetch("/api/v1/pm-schedule/generate",{method:"POST",headers:{"Content-Type":"application/json"},body:"{}"}).then(r => (r as any).data ?? r),
