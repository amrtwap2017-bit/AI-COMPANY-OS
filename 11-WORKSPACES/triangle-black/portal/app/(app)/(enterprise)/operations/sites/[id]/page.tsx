      ...
      <div className="tb-hero" style={{background:"linear-gradient(135deg, #221D1A 0%, #221D1A 100%)"}}>
        <div className="tb-hero-inner">
          ...
          <div className="tb-grid-4 mt-6">
            {[{label:"Assets",value:siteAssets.length,color:"#5B7C8C"},{label:"Operational",value:siteAssets.filter(a=>a.status==="Operational").length,color:"#547C4D"},{label:"In Fault",value:siteAssets.filter(a=>a.status==="In Fault").length,color:siteAssets.filter(a=>a.status==="In Fault").length>0?"#A84A3D":"#547C4D"},{label:"Categories",value:[...new Set(siteAssets.map(a=>a.category).filter(Boolean))].length,color:"#8D7443"}].map((k,i)=>(
