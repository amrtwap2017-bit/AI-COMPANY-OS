...
export default function Page() {
  ...
  const columns = [
    { key:"title", label:"Workflow", render:(r:any)=>(<span className="text-sm text-slate-700">{String(r["title"]??"—")}</span>) },
    ...
    { key:"created_at", label:"Started", render:(r:any)=>(<span className="text-sm text-slate-700">{String(r["created_at"]??"—")}</span>) },
  ];
  return (
    <PageWrapper>
      <Breadcrumb/>
      <PageHeader title="Workflows" subtitle={`${items.length} records`} badge="WF"
