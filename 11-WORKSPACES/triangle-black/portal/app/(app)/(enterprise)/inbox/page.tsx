...
export default function InboxPage() {
  ...
  const { data: notifRaw, isLoading } = useQuery(
    ["inbox-notifs-v2"],
    () => authFetch("/api/v1/notifications/?limit=50").then(r=>r.json()),
    { staleTime: 60000, refetchOnWindowFocus: false }
  );
  const { data: actRaw } = useQuery(
    ["inbox-activity-v2"],
    () => authFetch("/api/v1/activity-feed?limit=15").then(r=>r.json()),
    { staleTime: 60000, refetchOnWindowFocus: false }
  );
      ...
      <div className="tb-canvas">
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
          <div className="tb-section">
            ...
            {isLoading ? (
              <div className="space-y-3 mt-3">{[1,2,3,4].map(i=><div key={i} className="h-14 bg-base-alt rounded-xl animate-pulse"/>)}</div>
            ) : unread.length===0 ? (
              <div className="tb-empty" style={{padding:"32px 0"}}><div className="tb-empty-icon" style={{fontSize:"2.5rem"}}>✅</div><div className="tb-empty-desc">Inbox clear</div></div>
            ) : (
              <div className="space-y-1 mt-3">
                {unread.slice(0,8).map((n,i)=>{
                  const meta=TYPE_META[n.type]||{icon:"🔔",color:"#A78BFA",label:"System",path:"/workspace"};
                  return (
                    <button key={i} onClick={()=>router.push(meta.path)} className="w-full flex items-start gap-3 p-3 rounded-xl bg-base-alt hover:bg-surface transition-colors text-left" style={{borderLeft:"3px solid "+meta.color}}>
                      <div style={{width:32,height:32,borderRadius:8,flexShrink:0,background:meta.color+"18",border:"1px solid "+meta.color+"30",display:"flex",alignItems:"center",justifyContent:"center",fontSize:"0.875rem"}}>{meta.icon}</div>
                      <div className="flex-1 min-w-0">
                        <div className="flex justify-between gap-2"><div className="text-sm font-semibold text-primary truncate">{n.title}</div><div className="text-xs text-tertiary flex-shrink-0">{fmtTimeAgo(n.created_at)}</div></div>
                        {n.message&&<div className="text-xs text-secondary mt-0.5 truncate">{n.message}</div>}
                      </div>
                    </button>
                  );
                })}
                {unread.length>8&&<button onClick={()=>router.push("/notifications")} className="w-full text-center text-xs text-brand py-2 hover:text-primary transition-colors">+{unread.length-8} more →</button>}
              </div>
            )}
          </div>
          <div className="tb-section">
            ...
            <div className="space-y-2 mt-3">
              {activities.length===0 ? (
                <div className="tb-empty" style={{padding:"32px 0"}}><div className="tb-empty-icon" style={{fontSize:"2rem"}}>📋</div><div className="tb-empty-desc">No recent activity</div></div>
              ) : activities.slice(0,10).map((act,i)=>(
                <button key={i} onClick={()=>act.path&&router.push(act.path)} className="w-full flex items-start gap-3 text-left p-2 rounded-xl hover:bg-base-alt transition-colors">
                  <div style={{width:32,height:32,borderRadius:8,flexShrink:0,background:act.color+"18",border:"1px solid "+act.color+"30",display:"flex",alignItems:"center",justifyContent:"center",fontSize:"0.875rem"}}>{act.icon}</div>
                  <div className="flex-1 min-w-0">
                    <div className="flex justify-between gap-2"><div className="text-sm font-semibold text-primary truncate">{act.title}</div><div className="text-xs text-tertiary flex-shrink-0">{fmtTimeAgo(act.time)}</div></div>
                    {act.description&&<div className="text-xs text-tertiary mt-0.5 truncate">{act.description}</div>}
                  </div>
                </button>
              ))}
            </div>
