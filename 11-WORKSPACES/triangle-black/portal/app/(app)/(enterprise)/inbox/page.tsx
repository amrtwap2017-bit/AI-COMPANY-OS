...
export default function InboxPage() {
  ...
  useEffect(() => {
    // Fetch notifications immediately, no React Query overhead
    ...
    // Fetch activity feed separately so notifications show first
    setTimeout(() => {
      ...
      <div className="tb-canvas">
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
          ...
          <div className="tb-section">
            ...
            {!nLoaded ? (
              ...
              <div className="tb-empty" style={{padding:"24px 0"}}>
                ...
                <div className="tb-empty-desc">Inbox clear — no unread alerts</div>
              </div>
            ) : (
                ...
                {unread.slice(0,6).map((n,i)=>{
                  ...
                  return (
                    <button key={i} onClick={()=>router.push(m.path)}
                      ...
                      <div style={{width:32,height:32,borderRadius:8,flexShrink:0,background:m.color+"18",display:"flex",alignItems:"center",justifyContent:"center",fontSize:"0.875rem"}}>
                        {m.icon}
                      </div>
          ...
          <div className="tb-section">
            ...
            {!aLoaded ? (
              <div className="space-y-2 mt-3">
                {[1,2,3].map(i=><div key={i} className="h-12 bg-base-alt rounded-xl animate-pulse"/>)}
              </div>
            ) : activities.length === 0 ? (
              <div className="tb-empty" style={{padding:"24px 0"}}>
              ...
              <div className="space-y-2 mt-3">
                {activities.map((act,i)=>(
                  <button key={i} onClick={()=>act.path && router.push(act.path)}
                    ...
                    <div style={{width:32,height:32,borderRadius:8,flexShrink:0,background:act.color+"18",display:"flex",alignItems:"center",justifyContent:"center",fontSize:"0.875rem"}}>
                      {act.icon}
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="flex justify-between gap-2">
