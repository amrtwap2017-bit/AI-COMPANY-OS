...
export default function Page() {
  ...
  const columns = [
    { key:"type", label:"Action", render:(r:any)=>(<span className="text-sm text-slate-700">{String(r["type"]??"—")}</span>) },
    { key:"reference", label:"Reference", render:(r:any)=>(<span className="text-sm text-slate-700">{String(r["reference"]??"—")}</span>) },
    { key:"requestor", label:"By", render:(r:any)=>(<span className="text-sm text-slate-700">{String(r["requestor"]??"—")}</span>) },
    { key:"created_at", label:"Date", render:(r:any)=>(<span className="text-sm text-slate-700">{String(r["created_at"]??"—")}</span>) },
  ];
  return (
    <PageWrapper>
      <Breadcrumb/>
      <PageHeader title="Action Center" subtitle={`${items.length} records`} badge="ACT"
