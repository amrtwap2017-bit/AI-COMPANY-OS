    ...
    <div className="tb-page">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-white dark:bg-slate-900 rounded-2xl border p-6">
          ...
          {lowStock.length===0?(<div className="text-center py-6 text-tertiary">✅ All items above minimum</div>):(
            lowStock.slice(0,6).map((s,i)=>(
              <div key={i} className="flex items-center justify-between p-3 mb-1 bg-red-50 dark:bg-red-900/20 rounded-xl">
                <div><div className="text-sm font-medium">{s.item_name}</div><div className="text-xs text-secondary">{s.warehouse_name}</div></div>
                <div className="text-right"><div className="font-black text-red-500">{Math.round(s.qty_on_hand)}</div><div className="text-xs text-tertiary">on hand</div></div>
              </div>
            ))
          )}
        </div>
          ...
          <div className="space-y-2">
            {suppliers.slice(0,6).map((s,i)=>(
              <div key={i} className="flex items-center justify-between p-2 bg-base-alt dark:bg-surface-alt rounded-lg">
                <div><div className="text-sm font-medium">{s.company_name}</div><div className="text-xs text-tertiary">{s.supplier_type}</div></div>
