...
export default function StockLevelsPage() {
  ...
  const { data: stockRaw, isLoading } = useQuery(["sl-stock"], () => authFetch("/api/v1/stock-balances/").then(r => (r as any).data ?? r));
  const { data: itemRaw } = useQuery(["sl-items"], () => authFetch("/api/v1/inventory-items-portal").then(r => (r as any).data ?? r));
  ...
  const lowStock = enriched.filter((s: any) =>s.is_low);
  const filtered = filter==="all" ? enriched : filter==="low" ? lowStock : enriched.filter((s: any) =>!s.is_low);
  return (
    <div className="min-h-screen bg-base">
      <div className="tb-hero" >
        <div className="tb-hero-inner">
          ...
          <div className="tb-grid-4 mt-6">
            {[{label:"Total Records",value:stocks.length,color:"#221D1A"},{label:"Low Stock",value:lowStock.length,color:lowStock.length>0?"#A84A3D":"#547C4D"},{label:"In Stock",value:enriched.filter((s: any) =>!s.is_low).length,color:"#547C4D"},{label:"Items",value:items.length,color:"#5B7C8C"}].map((k: any, i: number) =>(
      ...
      <div className="tb-canvas">
        ...
        <div className="tb-section">
          ...
          {isLoading ? <div className="space-y-3">{[1,2,3,4].map((i: any) =><div key={i} className="h-12 bg-base-alt rounded-xl animate-pulse"/>)}</div>
          : <div className="tb-table" style={{borderRadius:12,overflow:"hidden"}}>
            <div className="tb-table-head" style={{gridTemplateColumns:"2fr 100px 80px 80px 100px"}}>
              {["Item","Category","On Hand","Min","Status"].map((h: any, i: number) =><div key={i} className="tb-table-head-cell" style={{textAlign:i>0?"center":"left"}}>{h}</div>)}
            </div>
