...
export default function Page() {
  const { data, isLoading, isError, error, refetch, isFetching } = useQuery({
    queryKey: ["supply-chain-supplier-invoices"],
  ...
  const columns = [
    { key:"invoice_number", label:"Invoice #", render:(r:any)=>(<span className="text-sm text-slate-700">{String(r["invoice_number"]??"—")}</span>) },
  ...
  return (
    <PageWrapper>
      <Breadcrumb/>
      <PageHeader title="Supplier Invoices" subtitle={`${items.length} records`} badge="SINV"
