...
export default function RFQManagementPage() {
  ...
  const createRFQ = useMutation(
    (payload)=>import("@/lib/hooks/useAuthFetch").then(m=>m.authFetch("/api/v1/rfq/",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(payload)})).then(r => (r as any).data ?? r),
  ...
  const { data: raw, isLoading } = useQuery(["rfq-list"],()=>authFetch("/api/v1/rfq/").then(r => (r as any).data ?? r),{staleTime:60000});
