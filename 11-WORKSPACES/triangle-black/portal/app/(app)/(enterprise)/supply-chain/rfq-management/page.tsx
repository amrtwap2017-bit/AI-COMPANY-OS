...
export default function RFQManagementPage() {
  ...
  const { data: raw, isLoading } = useQuery(["rfq-mgmt"], () => authFetch("/api/v1/rfq/").then(r=>r.json()), { staleTime:60000 });
      ...
      <div className="tb-hero" style={{background:"linear-gradient(135deg, #0F172A 0%, #0D1A12 100%)"}}>
        <div className="tb-hero-inner">
          ...
          <div className="tb-grid-4 mt-6">
            {[{label:"Total",value:rfqs.length,color:"#F1F5F9"},{label:"Active",value:rfqs.filter(r=>r.status==="sent").length,color:"#60A5FA"},{label:"With Quotes",value:rfqs.filter(r=>r.status==="responses_received").length,color:"#FBBF24"},{label:"Awarded",value:rfqs.filter(r=>r.status==="awarded").length,color:"#34D399"}].map((k,i)=>(
        ...
        <div className="tb-section">
          <div className="flex gap-2 mb-4 flex-wrap">
            {["all","draft","sent","responses_received","evaluated","awarded"].map(f=>(
              <button key={f} onClick={()=>setFilter(f)} className={"tb-pill "+(filter===f?"tb-pill--active":"")}>{f==="all"?"All":f.replace("_"," ")}</button>
            ))}
          </div>
          ...
          : filtered.length===0 ? (
            ...
            <div className="space-y-2">
              {filtered.map((r,i)=>{
                ...
                return (
                  <button key={i} onClick={()=>router.push("/supply-chain/rfq/"+r.id)} className="w-full flex items-center gap-4 p-4 rounded-xl bg-base-alt hover:bg-surface transition-colors text-left border border-transparent hover:border-border">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1">
                        <div className="text-sm font-bold text-primary truncate">{r.title}</div>
                        <span className="tb-badge flex-shrink-0" style={{background:sc+"18",color:sc,border:"1px solid "+sc+"30",fontSize:"0.5rem"}}>{(r.status||"").replace("_"," ")}</span>
                      </div>
