...
export default function SuppliersPage() {
  ...
  const { data: suppRaw, isLoading } = useQuery(
    ["supp-list"],
    () => authFetch("/api/v1/suppliers/").then(r => r.data ?? r),
    { refetchInterval: 60000 }
  );
  const { data: poRaw } = useQuery(["supp-pos"], () => authFetch("/api/v1/purchase-orders-portal").then(r => r.data ?? r));
  const { data: prRaw } = useQuery(["supp-prs"], () => authFetch("/api/v1/purchase-requests-portal").then(r => r.data ?? r));
