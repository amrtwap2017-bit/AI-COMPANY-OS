...
export default function Page() {
  ...
  const columns = [
    ...
    { key:"performance", label:"Performance", render:(r:any)=>(<span className="text-sm text-slate-700">{String(r["performance"]??"—")}</span>) },
    { key:"risk", label:"Risk", render:(r:any)=>(<span className="text-sm text-slate-700">{String(r["risk"]??"—")}</span>) },
  ];
  return (
    <PageWrapper>
      <Breadcrumb/>
      <PageHeader title="Vendor 360°" subtitle={`${items.length} records`} badge="VDR"
