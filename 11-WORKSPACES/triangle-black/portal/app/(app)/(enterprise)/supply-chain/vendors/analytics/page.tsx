...
export default function Page() {
  const { data, isLoading, isError, error, refetch, isFetching } = useQuery({
    queryKey: ["-api-v1-inventory-vendors"],
  ...
  const columns = [
    ...
    { key:"spend", label:"Spend", render:(r:any)=>(<span className="text-sm text-slate-700">{String(r["spend"]??"—")}</span>) },
    { key:"orders", label:"Orders", render:(r:any)=>(<span className="text-sm text-slate-700">{String(r["orders"]??"—")}</span>) },
    { key:"score", label:"Score", render:(r:any)=>(<span className="text-sm text-slate-700">{String(r["score"]??"—")}</span>) },
  ];
  return (
    <PageWrapper>
      <Breadcrumb/>
      <PageHeader title="Vendor Analytics" subtitle={`${items.length} records`} badge="VAN"
