...
export default function Page() {
  const { data, isLoading, isError, error, refetch, isFetching } = useQuery({
    queryKey: ["supply-chain-workbench"],
  ...
  const columns = [
    { key:"task", label:"Task", render:(r:any)=>(<span className="text-sm text-slate-700">{String(r["task"]??"—")}</span>) },
  ...
  return (
    <PageWrapper>
      <Breadcrumb/>
      <PageHeader title="SC Workbench" subtitle={`${items.length} records`} badge="WB"
