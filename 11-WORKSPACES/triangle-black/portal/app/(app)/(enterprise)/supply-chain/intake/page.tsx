    ...
    <PageWrapper>
      ...
      {!result && (
        <div className="space-y-6">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {toArr(CHANNELS).map(c => (                      <button key=c.key}
                        onClick={() => setChannel(c.key)}
                        className={`px-3 py-1.5 text-sm rounded-lg border transition-all                           $channel === c.key ? "bg-blue-600 text-white border-blue-600" : "border-slate-200 hover:border-slate-300"}`}>
                        {c.label}
                      </button>
                    ))}
                  </div>
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    {toArr(URGENCY).map(u => (                      <label key=u.key}
                        className={`flex items-center gap-3 p-3 rounded-lg border cursor-pointer transition-all                           $urgency === u.key ? u.color + " border-2" : "border-slate-200 hover:bg-slate-50"}`}>
          ...
          >
            {parse.isPending
              ? <><Loader2 className="w-5 h-5 animate-spin" /> Analyzing request...</>              : <><Send className="w-5 h-5" /> Submit Request - AI Will Handle The Rest</>}
          </button>
        </div>
      )}
      ...
      {/* Step 2: Results */}
      {result && !prCreated && (
        <div className="space-y-6">          /* Mentor Guidance */}
          {result.mentor_guidance?.length > 0 &(
            <div className="space-y-3">
              {result.mentor_guidance.map((g: any, i: number) => (                <div key=i} className={`flex items-start gap-3 p-4 rounded-xl border
                  $g.type === "urgent_sourcing" ? "bg-red-50 border-red-200" :                    g.type === "pr_required" ? "bg-amber-50 border-amber-200" :                     "bg-emerald-50 border-emerald-200"}`}>
          ...
          <div className="grid grid-cols-3 gap-4">
            {[               label: "Available in Stock",  value: result?.summary?.total || result?.summary?.count || 0?.available ?? 0,   color: "text-emerald-600", icon: CheckCircle },
              ...
              <div key={s.label} className="bg-white border border-slate-200 rounded-xl p-4 text-center">
                <s.icon className={`w-5 h-5 mx-auto mb-2 $s.color}`} />                 <div className={`text-2xl font-bold $s.color}`}>{s.value}</div>
                <div className="text-xs text-slate-500 mt-1">{s.label}</div>
              </div>
            ))}
          </div>

          {/* Inventory Results */}           <SectionCard title={`Inventory Check ($result.inventory_results?.length} items)`}>
            <div className="space-y-3">
              {result.inventory_results?.map((r: any, i: number) => (                <div key=i} className="p-4 bg-slate-50 rounded-xl border border-slate-200">
                  <div className="flex items-start justify-between gap-3">
                    ...
                    </div>                     <span classNam={`px-2 py-0.5 rounded text-xs font-semibold flex-shrink-0                      $STATUS_COLORS[r.stock_status] ?? "bg-slate-100 text-slate-600"}`}>
          ...
          {result.vendor_recommendations?.length > 0 && (
            <SectionCard title="Vendor Recommendations (AI Ranked)">
              <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">                {result.vendor_recommendations?.slice(0,3).map((v: any, i: number) => (                   <div key=i} className={`p-4 rounded-xl border $i === 0 ? "border-blue-300 bg-blue-50" : "border-slate-200 bg-white"}`}>
                    {i === 0 && (
          ...
          <div className="flex gap-3">
            ...
            {result.summary?.needs_pr && (
              <button                onClick=() => createPR.mutate()}
              ...
              >
                {createPR.isPending
                  ? <><Loader2 className="w-4 h-4 animate-spin" /> Creating PR...</>                  : <><ArrowRight className="w-4 h-4" /> Create Purchase Request & Start Approval</>}
              </button>
            )}
            {!result.summary?.needs_pr && (
      ...
      {prCreated && (
        <div className="text-center py-12">
          ...
          <h2 className="text-2xl font-bold text-slate-800 mb-2">Purchase Request Created!</h2>          <p className="text-slate-600 mb-6">prCreated.message}</p>

          <div className="max-w-md mx-auto space-y-3 text-left">
            <div className="p-4 bg-blue-50 border border-blue-200 rounded-xl">
              <div className="text-xs font-semibold text-blue-600 uppercase mb-2">Approval Chain Started</div>
              {prCreated.approval_chain?.map((step: any) => (                <div key=step.step} className="flex items-center gap-3 py-1">                   <div className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold                    $step.status === "pending" ? "bg-blue-600 text-white" : "bg-slate-200 text-slate-500"}`}>
                    {step.step}
                  </div>
                  <span className="text-sm text-slate-700">{step.role}</span>                   <span className={`text-xs ml-auto capitalize $step.status === "pending" ? "text-blue-600 font-semibold" : "text-slate-400"}`}>
                    {step.status}
                  </span>
                </div>
              ))}
