...
export default function PurchaseOrdersV2Page() {
  ...
  const { data: raw, isLoading } = useQuery(["pov2-list"], () => authFetch("/api/v1/purchase-orders-v2/").then(r=>r.json()), { staleTime:60000 });
  const pos = toArr(raw);
      ...
      <div className="tb-hero" style={{background:"linear-gradient(135deg, #0F172A 0%, #0D1A1A 100%)"}}>
        <div className="tb-hero-inner">
          ...
          <div className="tb-grid-4 mt-6">
            {[{label:"Total POs",value:pos.length,color:"#F1F5F9"},{label:"Pending",value:pos.filter(p=>p.status==="pending_approval").length,color:"#FBBF24"},{label:"Approved",value:pos.filter(p=>p.status==="approved").length,color:"#34D399"},{label:"Total Value",value:fmtEGP(pos.reduce((s,p)=>s+Number(p.total_amount||0),0)),color:"#A78BFA"}].map((k,i)=>(
        ...
        <div className="tb-section">
          <div className="flex gap-2 mb-4 flex-wrap">
            {["all","draft","pending_approval","approved","sent","received","paid"].map(f=>(
              <button key={f} onClick={()=>setFilter(f)} className={"tb-pill "+(filter===f?"tb-pill--active":"")}>{f==="all"?"All":f.replace("_"," ")}{f!=="all"&&<span className="ml-1 opacity-60">{pos.filter(p=>p.status===f).length}</span>}</button>
            ))}
          </div>
          <div className="tb-flex-between mb-3"><div className="text-sm text-secondary">{filtered.length} POs</div><div className="text-sm font-bold text-emerald-400">{fmtEGP(totalValue)}</div></div>
          {isLoading ? <div className="space-y-3">{[1,2,3].map(i=><div key={i} className="h-14 bg-base-alt rounded-xl animate-pulse"/>)}</div>
          : filtered.length===0 ? <div className="tb-empty"><div className="tb-empty-icon">📦</div><div className="tb-empty-title">No purchase orders</div></div>
          : <div className="tb-table" style={{borderRadius:12,overflow:"hidden"}}>
            <div className="tb-table-head" style={{gridTemplateColumns:"1.5fr 130px 100px 120px 110px"}}>
            ...
            {filtered.map((po,i)=>{
              ...
              return (
                <button key={i} onClick={()=>router.push("/supply-chain/purchase-orders-v2/"+po.id)} className="tb-table-row" style={{gridTemplateColumns:"1.5fr 130px 100px 120px 110px"}}>
                  <div className="min-w-0 pr-4"><div className="text-sm font-semibold text-primary truncate">{po.po_number||po.id?.slice(0,14)}</div><div className="text-xs text-tertiary truncate">{po.vendor_name||"—"}</div></div>
                  <div className="text-center"><span className="tb-badge" style={{background:sc+"18",color:sc,border:"1px solid "+sc+"30",fontSize:"0.5625rem"}}>{(po.status||"").replace("_"," ")}</span></div>
