...
export default function InventoryPage() {
  ...
  const { data: itemRaw, isLoading } = useQuery(["inv-items"],  ()=>authFetch("/api/v1/inventory-items-portal").then(r => r.data ?? r),{refetchInterval:60000});
  const { data: stockRaw }           = useQuery(["inv-stock"],  ()=>authFetch("/api/v1/stock-balances/").then(r => r.data ?? r));
  const { data: whRaw }              = useQuery(["inv-wh"],     ()=>authFetch("/api/v1/warehouses-portal").then(r => r.data ?? r));
