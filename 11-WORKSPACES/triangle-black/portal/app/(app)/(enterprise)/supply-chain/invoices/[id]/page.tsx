...
export default function InvoiceDetailPage() {
  ...
  const { data: inv, isLoading } = useQuery(["invoice-detail",id], ()=>authFetch(`/api/v1/supplier-invoices/${id}`).then(r => (r as any).data ?? r), {staleTime:30000});
  const matchMut   = useMutation(()=>authFetch(`/api/v1/supplier-invoices/${id}/match`,{method:"POST"}).then(r => (r as any).data ?? r), {onSuccess:()=>qc.invalidateQueries(["invoice-detail",id])});
  const approveMut = useMutation((action)=>authFetch(`/api/v1/supplier-invoices/${id}/approve`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({action,approved_by:"amr@triangleblack.com"})}).then(r => (r as any).data ?? r), {onSuccess:()=>qc.invalidateQueries(["invoice-detail",id])});
  const payMut     = useMutation((payload)=>authFetch(`/api/v1/supplier-invoices/${id}/pay`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(payload)}).then(r => (r as any).data ?? r), {onSuccess:()=>{qc.invalidateQueries(["invoice-detail",id]);setShowPay(false);setPayForm({amount:"",method:"bank_transfer",reference:"",notes:""});}});
