...
export default function InvoicesPage() {
  ...
  const { data: raw, isLoading } = useQuery(
    ...
    () => authFetch(`/api/v1/supplier-invoices/${filter !== "all" ? `?status=${filter}` : ""}`).then(r => (r as any).data ?? r),
    { staleTime: 30000 }
  );
  const { data: dash } = useQuery(["invoice-dash"], () => authFetch("/api/v1/supplier-invoices/dashboard").then(r => (r as any).data ?? r), { staleTime: 60000 });
