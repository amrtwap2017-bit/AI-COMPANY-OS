...
export default function WarehouseDetailPage() {
  ...
  const { data: whRaw, isLoading } = useQuery(["wh-d", id], () => authFetch("/api/v1/warehouses-portal").then(r => r.data ?? r));
  const { data: stockRaw } = useQuery(["wh-d-stock"], () => authFetch("/api/v1/stock-balances/").then(r => r.data ?? r));
  const { data: itemRaw } = useQuery(["wh-d-items"], () => authFetch("/api/v1/inventory-items-portal").then(r => r.data ?? r));
