...
export default function ConnectSignalsPage() {
  ...
  const { data: sigRaw }  = useQuery(["cs-signals"], () => authFetch("/api/v1/ai/signals").then(r => (r as any).data ?? r));
  const { data: twin }    = useQuery(["cs-twin"],    () => authFetch("/api/v1/twin/state").then(r => (r as any).data ?? r));
  const { data: actRaw }  = useQuery(["cs-act"],     () => authFetch("/api/v1/activity-feed?limit=20").then(r => (r as any).data ?? r));
