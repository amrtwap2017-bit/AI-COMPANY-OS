...
export default function MyDayPage() {
  ...
  if (l1 || l2 || l3) return <div className="p-6 text-tertiary text-center">Loading your day...</div>;
      ...
      <div className="flex items-start justify-between">
        ...
        <div className="flex gap-3">
          <div className="bg-surface border border-border rounded-2xl px-5 py-3 text-center">
            ...
            <div className="text-xs text-secondary">Tasks Today</div>
          </div>
          <div className={`rounded-2xl border px-5 py-3 text-center ${score>=95?"bg-emerald-50 border-emerald-200":"bg-amber-50 border-amber-200"}`}>
            ...
            <div className="text-xs text-secondary">Twin Score</div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <div className="xl:col-span-2 bg-surface border border-border rounded-2xl p-5">
          <div className="flex items-center justify-between mb-4">
            <h2 className="font-bold text-primary">Today's Task Queue ({allTasks.length})</h2>
          ...
          {allTasks.length === 0 ? (
            <div className="text-center py-12">
              ...
              <div className="text-tertiary text-sm mt-1">No tasks require your attention today</div>
            </div>
          ) : (
              ...
              {allTasks.map((task,i) => (
                <button key={i} onClick={() => router.push(task.path)}
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-xs text-secondary mt-0.5">{task.type} · {task.status}</div>
                  </div>
                  <div className="text-right flex-shrink-0">
                    {task.due && <div className={`text-xs font-medium ${new Date(task.due)<now?"text-red-500":"text-tertiary"}`}>{fmtDate(task.due)}</div>}
                    <div className="text-tertiary text-xs">→</div>
                  </div>
                </button>
              ))}
            </div>
        ...
        <div className="space-y-4">
          ...
          <div className="bg-surface border border-border rounded-2xl p-5">
            <div className="flex items-center justify-between mb-3">
              <h2 className="font-bold text-primary text-sm">📅 PM Due This Week</h2>
              <button onClick={() => router.push("/maintenance/pm-plans")} className="text-xs text-amber-500">All →</button>
            </div>
            {dueSoonPMs.length === 0 ? (
              <div className="text-center py-4 text-tertiary text-xs">✅ No PM plans due this week</div>
            ) : (
              <div className="space-y-2">
          ...
          <div className="bg-surface border border-border rounded-2xl p-5">
            <div className="flex items-center justify-between mb-3">
              <h2 className="font-bold text-primary text-sm">🔔 Unread Alerts ({unreadNotifs.length})</h2>
              <button onClick={() => router.push("/inbox")} className="text-xs text-amber-500">Inbox →</button>
            </div>
            {unreadNotifs.length === 0 ? (
              <div className="text-center py-4 text-tertiary text-xs">✅ All caught up</div>
            ) : (
              <div className="space-y-2">
                {unreadNotifs.map((n,i)=>(
                  <div key={i} className="flex items-start gap-2 p-2 bg-base-alt dark:bg-surface-alt rounded-lg">
                    <div className="w-1.5 h-1.5 bg-amber-500 rounded-full mt-1.5 flex-shrink-0"/>
                    <div className="min-w-0">
                      <div className="text-xs font-semibold text-primary truncate">{n.title}</div>
                      <div className="text-[10px] text-tertiary truncate">{n.message}</div>
                    </div>
                  </div>
                ))}
              </div>
          ...
          {/* Quick create */}
          <div className="bg-surface border border-border rounded-2xl p-5">
            <h2 className="font-bold text-primary text-sm mb-3">⚡ Quick Create</h2>
            <div className="space-y-2">
              {[
                ...
                <button key={i} onClick={()=>router.push(a.path)}
                  className="w-full flex items-center gap-2.5 p-2.5 bg-base-alt dark:bg-surface-alt rounded-xl hover:bg-amber-50 dark:hover:bg-amber-900/20 text-left transition-colors">
                  <span className="text-base">{a.icon}</span>
                  <span className="text-xs font-semibold text-slate-700 dark:text-tertiary">{a.label}</span>
                </button>
              ))}
            </div>
          </div>
