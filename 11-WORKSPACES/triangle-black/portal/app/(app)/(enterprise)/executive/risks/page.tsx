...
export default function RisksPage() {
  ...
  const { data: woRaw }   = useQuery(["rsk-wos"],   () => authFetch("/api/v1/work-orders/").then(r => (r as any).data ?? r));
  const { data: contRaw } = useQuery(["rsk-conts"], () => authFetch("/api/v1/contracts/").then(r => (r as any).data ?? r));
  const { data: invRaw }  = useQuery(["rsk-inv"],   () => authFetch("/api/v1/invoices/").then(r => (r as any).data ?? r));
  const { data: pmRaw }   = useQuery(["rsk-pms"],   () => authFetch("/api/v1/maintenance/pm-plans/").then(r => (r as any).data ?? r));
