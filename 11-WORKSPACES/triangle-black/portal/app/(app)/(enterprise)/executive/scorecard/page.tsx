...
export default function ScorecardPage() {
  ...
  const { data: twin }   = useQuery(["sc2-twin"],  () => authFetch("/api/v1/twin/state").then(r => r.data ?? r));
  const { data: woRaw }  = useQuery(["sc2-wos"],   () => authFetch("/api/v1/work-orders/").then(r => r.data ?? r));
  const { data: invRaw } = useQuery(["sc2-inv"],   () => authFetch("/api/v1/invoices/").then(r => r.data ?? r));
  const { data: pmRaw }  = useQuery(["sc2-pms"],   () => authFetch("/api/v1/maintenance/pm-plans/").then(r => r.data ?? r));
