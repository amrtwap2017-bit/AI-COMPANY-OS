...
export default function ExecutiveReportsPage() {
  ...
  const { data: daily }    = useQuery(["rpt-daily"],     ()=>authFetch("/api/v1/reports/daily-summary").then(r => (r as any).data ?? r), {staleTime:60000});
  const { data: woReport } = useQuery(["rpt-wo"],        ()=>authFetch("/api/v1/reports/work-orders").then(r => (r as any).data ?? r),   {staleTime:120000});
  const { data: assetRpt } = useQuery(["rpt-assets"],    ()=>authFetch("/api/v1/reports/assets").then(r => (r as any).data ?? r),        {staleTime:120000});
  const { data: contRpt }  = useQuery(["rpt-contracts"], ()=>authFetch("/api/v1/reports/contracts").then(r => (r as any).data ?? r),     {staleTime:120000});
