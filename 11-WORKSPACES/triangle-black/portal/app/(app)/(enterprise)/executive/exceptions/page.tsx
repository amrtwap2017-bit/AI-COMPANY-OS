...
export default function ExceptionsPage() {
  ...
  const { data: woRaw }    = useQuery(["exc-wos"],   ()=>authFetch("/api/v1/work-orders/").then(r => r.data ?? r));
  const { data: contRaw }  = useQuery(["exc-conts"], ()=>authFetch("/api/v1/contracts/").then(r => r.data ?? r));
  const { data: pmRaw }    = useQuery(["exc-pms"],   ()=>authFetch("/api/v1/maintenance/pm-plans/").then(r => r.data ?? r));
  const { data: notifRaw } = useQuery(["exc-notifs"],()=>authFetch("/api/v1/notifications-portal?limit=100").then(r => r.data ?? r));
