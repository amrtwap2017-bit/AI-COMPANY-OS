...
export default function ExecutiveDashboardPage() {
  ...
  const { data: dash, isLoading } = useQuery(["exec-dashboard"], ()=>authFetch("/api/v1/executive/dashboard").then(r => (r as any).data ?? r), {staleTime:60000,refetchInterval:120000});
  const { data: procDash } = useQuery(["exec-procurement"], ()=>authFetch("/api/v1/procurement/dashboard").then(r => (r as any).data ?? r), {staleTime:60000});
  const { data: timeDash } = useQuery(["exec-time"], ()=>authFetch("/api/v1/time-entries/summary").then(r => (r as any).data ?? r), {staleTime:60000});
  const { data: slaDash } = useQuery(["exec-sla"], ()=>authFetch("/api/v1/sla/dashboard").then(r => (r as any).data ?? r), {staleTime:60000});
