...
export default function Page() {
  ...
  const { data, isLoading, isError, error, refetch, isFetching } = useQuery({
    ...
    queryFn:  () => authFetchJSON("/api/v1/actions/executive/portfolio"),
  ...
  const columns = [
    { key:"name", label:"Contract", render:(r:any)=>(<span className="text-sm text-slate-700">{String(r["name"]??"—")}</span>) },
  ...
  return (
    <PageWrapper>
      <Breadcrumb/>
      <PageHeader title="Executive Portfolio" subtitle={`${items.length} records`} badge="EXEC"
