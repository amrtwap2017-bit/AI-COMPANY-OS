...
export default function PortfolioPage() {
  ...
  const { data: contRaw } = useQuery(["pf-conts"], () => authFetch("/api/v1/contracts/").then(r => r.data ?? r));
  const { data: projRaw } = useQuery(["pf-projs"], () => authFetch("/api/v1/projects/").then(r => r.data ?? r));
  const { data: invRaw }  = useQuery(["pf-inv"],   () => authFetch("/api/v1/invoices/").then(r => r.data ?? r));
