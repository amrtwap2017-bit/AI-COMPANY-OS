...
export default function WorkbenchPage() {
  ...
  const { data: twin }    = useQuery(["wb-twin"],   () => authFetch("/api/v1/twin/state").then(r => (r as any).data ?? r));
  const { data: dash }    = useQuery(["wb-dash"],   () => authFetch("/api/v1/dashboard/summary").then(r => (r as any).data ?? r));
  const { data: actRaw }  = useQuery(["wb-act"],    () => authFetch("/api/v1/activity-feed?limit=10").then(r => (r as any).data ?? r));
