...
export default function DailyReviewPage() {
  ...
  const { data: woRaw }   = useQuery(["dr-wos"],   () => authFetch("/api/v1/work-orders/").then(r => (r as any).data ?? r));
  const { data: notifRaw }= useQuery(["dr-notifs"],() => authFetch("/api/v1/notifications-portal?limit=50").then(r => (r as any).data ?? r));
  const { data: twin }    = useQuery(["dr-twin"],  () => authFetch("/api/v1/twin/state").then(r => (r as any).data ?? r));
  const { data: actRaw }  = useQuery(["dr-act"],   () => authFetch("/api/v1/activity-feed?limit=15").then(r => (r as any).data ?? r));
