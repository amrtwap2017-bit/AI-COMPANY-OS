    ...
    <div className="tb-page">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-surface border border-border rounded-2xl p-6">
          <h2 className="font-bold text-primary mb-4">🚨 Needs Action Today</h2>
          {openCritical.length === 0 ? (
            <div className="text-center py-6 text-tertiary text-sm">Nothing critical today ✅</div>
        ...
        {/* PM due this week */}
        <div className="bg-surface border border-border rounded-2xl p-6">
          <h2 className="font-bold text-primary mb-4">📅 PM Due This Week</h2>
          {dueSoon.length === 0 ? (
            <div className="text-center py-6 text-tertiary text-sm">No PM plans due this week ✅</div>
        ...
        <div className="bg-surface border border-border rounded-2xl p-6">
          <h2 className="font-bold text-primary mb-4">🔔 Unread Alerts</h2>
          {unreadNotifs.length === 0 ? (
            <div className="text-center py-6 text-tertiary text-sm">All caught up ✅</div>
          ) : unreadNotifs.map((n: any, i: number) => (
            <div key={i} className="p-3 mb-2 bg-base-alt dark:bg-surface-alt rounded-xl">
      ...
      {/* Domain summary */}
      <div className="bg-surface border border-border rounded-2xl p-6">
        <h2 className="font-bold text-primary mb-4">Platform at a Glance</h2>
        <div className="grid grid-cols-3 md:grid-cols-6 gap-4 text-center">
          {[
            ...
            <button key={i} onClick={() => router.push(item.path)}
              className="p-4 bg-base-alt dark:bg-surface-alt rounded-xl hover:bg-amber-50 dark:hover:bg-amber-900/20 transition-colors">
