...
export default function Page() {
  const { data, isLoading, isError, error, refetch, isFetching } = useQuery({
    queryKey: ["-api-v1-actions-reports-dashboard"],
    queryFn:  () => authFetchJSON("/api/v1/actions/reports/dashboard"),
  ...
  const columns = [
    ...
    { key:"leads", label:"Leads", render:(r:any)=>(<span className="text-sm text-slate-700">{String(r["leads"]??"—")}</span>) },
    { key:"revenue", label:"Revenue", render:(r:any)=>(<span className="text-sm text-slate-700">{String(r["revenue"]??"—")}</span>) },
    { key:"status", label:"Status", render:(r:any)=>(<span className="text-sm text-slate-700">{String(r["status"]??"—")}</span>) },
  ];
  return (
    <PageWrapper>
      <Breadcrumb/>
      <PageHeader title="Reports" subtitle={`${items.length} records`} badge="RPT"
