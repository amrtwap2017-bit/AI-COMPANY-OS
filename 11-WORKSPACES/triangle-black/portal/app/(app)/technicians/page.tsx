...
export default function Page() {
  ...
  const columns = [
    ...
    { key:"phone", label:"Phone", render:(r:any)=>(<span className="text-sm text-slate-700">{String(r["phone"]??"—")}</span>) },
    { key:"is_active", label:"Active", render:(r:any)=>(<span className="text-sm text-slate-700">{String(r["is_active"]??"—")}</span>) },
  ];
  return (
    <PageWrapper>
      <Breadcrumb/>
      <PageHeader title="Technicians" subtitle={`${items.length} records`} badge="TECH"
