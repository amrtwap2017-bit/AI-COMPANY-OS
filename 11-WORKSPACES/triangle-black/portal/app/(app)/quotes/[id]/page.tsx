      ...
      <div>
        ...
        <div className="flex items-start justify-between flex-wrap gap-4">
          <div>
            ...
            {quote.description && <p className="text-gray-500 mt-1">{quote.description}</p>}
          </div>
              ...
              <tbody>
                {quote.items.map((item, i) => (
                  <tr key={i} className="border-t border-gray-50">
                    ...
                    <td className="px-6 py-4 text-right text-gray-500">{formatEGP(item.unit_price)}</td>
                    <td className="px-6 py-4 text-right font-semibold text-gray-900">{formatEGP(item.total)}</td>
                  </tr>
                ))}
              </tbody>
              <tfoot>
                <tr className="border-t-2 border-gray-200 bg-gray-50">
                  <td colSpan={3} className="px-6 py-4 font-bold text-gray-900 text-right">Total</td>
                  <td className="px-6 py-4 font-bold text-[#1B2B4B] text-right text-lg">{formatEGP(quote.total)}</td>
                </tr>
              </tfoot>
            </table>
          </Card>
          ...
          <Card>
            ...
            <dl className="grid grid-cols-2 gap-4 text-sm">
              ...
              <div>
                ...
                <dd><Badge color={cfg.color} bg={cfg.bg}>{cfg.label}</Badge></dd>
              </div>
              <div>
          ...
          <Card>
            ...
            <div className="space-y-3">
              <Button className="w-full" variant="secondary"
                ...
                onClick={() => doAction("submit", () => quotesApi.submit(id))}
              >
              ...
              <Button className="w-full" variant="success"
                ...
                onClick={() => doAction("approve", () => quotesApi.approve(id))}
              >
              ...
              <Button className="w-full" variant="danger"
                ...
                onClick={() => doAction("reject", () => quotesApi.reject(id))}
              >
            ...
            <div className="mt-6 pt-4 border-t border-gray-100">
              ...
              {(["draft","review","sent","approved"] as QuoteStatus[]).map((s) => {
                ...
                const done = ["review","sent","approved"].includes(quote.status) &&
                  (s === "draft" || (s === "review" && ["sent","approved"].includes(quote.status))
                  || (s === "sent" && quote.status === "approved"));
                return (
                  <div key={s} className="flex items-center gap-2 mb-1.5">
                    <div className={`w-2 h-2 rounded-full ${active ? "bg-[#1B2B4B]" : done ? "bg-green-500" : "bg-gray-200"}`} aria-hidden="true" />
                    <span className={`text-xs ${active ? "font-semibold text-[#1B2B4B]" : done ? "text-green-600" : "text-gray-400"}`}>
                      {c.label}
                    </span>
                  </div>
                );
