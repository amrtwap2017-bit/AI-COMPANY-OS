    ...
    <div className="p-6 space-y-6 bg-slate-50 dark:bg-slate-950 min-h-screen">
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1.5">{invoices.length} invoices · {collectionRate}% collection rate · {fmtEGP(pendingValue+overdueValue)} outstanding</p>
        </div>
      ...
      <div className="flex gap-3 flex-wrap">
        ...
        <div className="text-xs text-slate-400 self-center">{filtered.length} invoices</div>
      </div>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 overflow-hidden">
        {filtered.length === 0 ? (
          <div className="text-center py-16">
            ...
            <div className="font-bold text-slate-900 dark:text-white text-lg">No invoices found</div>
          </div>
        ) : (
          <>
            <div className="grid grid-cols-[1fr_150px_130px_130px_120px] bg-slate-50 dark:bg-slate-800/50 px-5 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wider">
              <div>Invoice</div>
            ...
            <div className="divide-y divide-slate-50 dark:divide-slate-800">
              {filtered.map((inv,i)=>(
                <button key={i} onClick={()=>router.push(`/invoices/${inv.id}`)}
                  className={`w-full grid grid-cols-[1fr_150px_130px_130px_120px] items-center px-5 py-4 text-left transition-colors group ${inv.status==="overdue"?"bg-red-50/30 dark:bg-red-900/5 hover:bg-red-50":"hover:bg-amber-50/50 dark:hover:bg-amber-900/10"}`}>
                  <div className="min-w-0 pr-4">
                    <div className="font-semibold text-sm text-slate-900 dark:text-white truncate group-hover:text-amber-600">{inv.invoice_number||`INV-${inv.id?.slice(0,8)}`}</div>
                    <div className="text-xs text-slate-400 mt-0.5">Invoice ID: {inv.id?.slice(0,12)}</div>
                  </div>
                  <div className="text-center">
                    <span className={`text-xs font-medium px-2.5 py-1 rounded-lg ${STATUS_BADGE[inv.status]||"bg-slate-100 text-slate-600"}`}>{inv.status||"—"}</span>
                  </div>
                  <div className="text-right text-sm font-bold text-slate-800 dark:text-slate-200">{fmtEGP(inv.total_amount)}</div>
                  <div className={`text-center text-xs ${inv.status==="overdue"?"text-red-500 font-bold":"text-slate-500"}`}>
                    {fmtDate(inv.due_date)}
                    {inv.status==="overdue" && <div className="text-[10px] text-red-400">OVERDUE</div>}
                  </div>
                  <div className="text-center text-xs text-slate-400">{fmtDate(inv.created_at)}</div>
                </button>
              ))}
            </div>
          </>
