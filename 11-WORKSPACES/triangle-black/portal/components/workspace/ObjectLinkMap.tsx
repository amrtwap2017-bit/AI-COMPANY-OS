...
export function ObjectLinkMap({ title, subtitle, nodes=[] }:any) {
  const tones:any = { warning:"border-amber-300 bg-amber-50", success:"border-emerald-300 bg-emerald-50", neutral:"border-stone-200 bg-slate-50" };
  return (
      ...
      <div className="grid grid-cols-2 lg:grid-cols-3 gap-3">
        {nodes.map((node:any,i:number)=>(
          <div key={i} className={"p-3 rounded-xl border "+(tones[node.tone]||tones.neutral)}>
            <p className="text-lg font-bold text-stone-900">{node.value}</p>
