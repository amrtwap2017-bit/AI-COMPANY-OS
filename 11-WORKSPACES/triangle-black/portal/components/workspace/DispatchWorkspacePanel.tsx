...
export function DispatchWorkspacePanel({ title, subtitle, lanes=[] }:any) {
  const tones:any = { warning:"border-amber-200 bg-amber-50", success:"border-emerald-200 bg-emerald-50", neutral:"border-stone-200 bg-slate-50" };
  return (
        ...
        {lanes.map((lane:any,i:number)=>(
          <div key={i} className="space-y-2">
            <div className="flex items-center justify-between mb-2">
              <p className="text-sm font-bold text-stone-800">{lane.title}</p>
