          ...
          {items.map((item, index) => (
            ...
            >
              <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                ...
                <div className="text-xs uppercase tracking-wide text-slate-500">{item.entityType}</div>
              </div>
              <div className="mt-2 text-sm text-slate-600">Target: {item.href}</div>
              <div className="mt-1 text-xs text-slate-500">Context: {item.contextPath || "—"}</div>
            </Link>
          ))}
        </div>
      )}
