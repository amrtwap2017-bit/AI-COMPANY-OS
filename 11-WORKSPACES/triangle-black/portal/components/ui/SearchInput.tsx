  ...
  return (
    <div className={"relative " + className}>
      ...
      <input
        ...
        {...props}
        className="w-full pl-9 pr-8 py-2 text-sm bg-white border border-border rounded-xl focus:outline-none focus:ring-2 focus:ring-amber-500/20 focus:border-amber-400 placeholder-slate-400 transition-all"
      />
      {hasValue && onClear ? (
        <button
          type="button"
          ...
          className="absolute right-2.5 top-1/2 -translate-y-1/2 w-5 h-5 flex items-center justify-center rounded-full text-tertiary hover:text-secondary hover:bg-surface-alt transition-colors"
        >
          <X className="w-3 h-3" />
        </button>
      ) : showHint ? (
