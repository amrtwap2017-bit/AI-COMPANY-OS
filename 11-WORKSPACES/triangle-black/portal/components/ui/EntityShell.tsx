...
export function EntityShell({
  ...
  return (
    <div className="space-y-0 -mt-6 -mx-6">
      {/* Entity Header */}
      <div className="bg-white border-b border-border px-6 pt-6 pb-0">
        {/* Back + breadcrumb */}
        {backHref && (
            ...
            {heroMetrics.map((m: any, i: number) => (
              <div key={i} className="flex items-center gap-2">
                <div>
                  ...
                  <div className={`text-lg font-bold ${m.color ? `text-${m.color}-600` : "text-primary"}`}>{m.value}</div>
                </div>
                {i < heroMetrics.length - 1 && <div className="w-px h-8 bg-surface-alt ml-2" />}
              </div>
            ))}
          </div>
        )}
