  ...
  return (
    <div className="flex flex-col h-full" aria-busy="true">
      <div className="px-6 py-4 border-b space-y-2"
        ...
        <div className="flex gap-4">{[...Array(4)].map((_: any, i: any) => <Sk key={i} className="h-8 w-24" />)}</div>
      </div>
          ...
          {[...Array(3)].map((_: any, i: any) => (
            <div key={i} className="rounded-xl border p-5 space-y-3"
              ...
              <div className="grid grid-cols-3 gap-4">
                {[...Array(6)].map((_: any, j: any) => <div key={j} className="space-y-1.5"><Sk className="h-3 w-20" /><Sk className="h-4 w-28" /></div>)}
              </div>
            </div>
          ))}
        </div>
        <div className="w-64 border-l p-4 space-y-3" style={{ borderColor:'var(--tb-border)' }}>
          {[...Array(5)].map((_: any, i: any) => <Sk key={i} className="h-10 rounded-lg" />)}
        </div>
      </div>
    </div>
  )
