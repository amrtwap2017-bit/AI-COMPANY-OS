...
from knowledge.search import KnowledgeSearch, SearchResult
...
class HybridKnowledgeSearch:
    ...
    def _vector_search(
        ...
        try:
            ...
            return [
                (
                    r["payload"].get("content", r["payload"].get("text", "")),
                    r["score"],
                    r["payload"].get("source", "knowledge"),
                )
                for r in raw
                if r["payload"].get("content") or r["payload"].get("text")
            ]
    ...
    def _keyword_search(
        ...
        try:
            ...
            for entry in entries:
                ...
                source = getattr(entry, "source", "knowledge")
                results.append((entry.content, score, source))
    ...
    def _rrf_combine(
        ...
        vector_source: dict[str, str]   = {}
        for rank, item in enumerate(vector_results):
            text, score = item[0], item[1]
            source = item[2] if len(item) > 2 else "knowledge"
            vector_rank[text]   = rank
            vector_score[text]  = score
            vector_source[text] = source
        ...
        for rank, item in enumerate(keyword_results):
            text, score = item[0], item[1]
            ksource = item[2] if len(item) > 2 else "knowledge"
            keyword_rank[text]   = rank
            keyword_score[text]  = score
            keyword_source[text] = ksource
        ...
        for text in all_texts:
            ...
            results.append(HybridSearchResult(
                ...
                text=text[:2000],
                source=vector_source.get(text, keyword_source.get(text, "knowledge")),
                title="",
                document_id=0,
                chunk_index=0,
            ))
