import os, subprocess, datetime, json, sys

ROOT  = "/home/amr/AI-COMPANY-OS"
LOG   = ROOT + "/tasks/logs/reliability_master.log"
TASKS = [
    ("R1_process_manager", ROOT + "/tasks/reliability/r1_process_manager.py"),
    ("R2_fix_imports",     ROOT + "/tasks/reliability/r2_fix_imports.py"),
    ("R3_bundle_optimize", ROOT + "/tasks/reliability/r3_bundle_optimize.py"),
    ("R4_data_seed",       ROOT + "/tasks/reliability/r4_data_seed.py"),
    ("R5_final",           ROOT + "/tasks/reliability/r5_final.py"),
]

def log(m):
    ts = datetime.datetime.now().strftime("%H:%M:%S")
    out = "["+ts+"] "+str(m)
    print(out, flush=True)
    open(LOG, "a").write(out+"\n")

filter_task = sys.argv[1] if len(sys.argv) > 1 else None
open(LOG,"w").close()
log("="*50)
log("AI COMPANY OS — RELIABILITY UPGRADE v2.2.0")
log("="*50)
summary = {}

tasks_to_run = [(n,p) for n,p in TASKS if not filter_task or filter_task in n]
log(f"Running {len(tasks_to_run)} tasks")

for name, path in tasks_to_run:
    log("\n"+"─"*40)
    log(f"RUNNING: {name}")
    if not os.path.exists(path):
        log("  SKIP")
        summary[name] = "SKIPPED"
        continue
    start = datetime.datetime.now()
    try:
        r = subprocess.run(["python3", path], capture_output=False, timeout=600)
        secs = (datetime.datetime.now() - start).seconds
        status = f"PASSED ({secs}s)" if r.returncode == 0 else "FAILED"
        log(f"  {status}")
        summary[name] = status
    except Exception as e:
        log(f"  ERROR: {e}")
        summary[name] = "ERROR"

log("\n"+"="*50)
log("RELIABILITY UPGRADE COMPLETE")
passed = sum(1 for v in summary.values() if "PASSED" in v)
log(f"Results: {passed}/{len(summary)} passed")
for n,s in summary.items():
    icon = "OK" if "PASSED" in s else "XX"
    log(f"  [{icon}] {n}: {s}")

with open(ROOT+"/tasks/logs/reliability_summary.json","w") as f:
    json.dump({"date":str(datetime.datetime.now()),"tasks":summary},f,indent=2)
log("\nVersion: v2.2.0")
log("Start:   bash START-SAFE.sh")
log("Watch:   bash WATCHDOG.sh &")
log("Check:   bash CHECK.sh")
